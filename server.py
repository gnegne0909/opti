...
def site_home():
    ...
    chatbot_on    = get_site_setting("chatbot_enabled","1") == "1"
...
<div id="chatbot-notif" style="position:fixed;bottom:84px;right:24px;background:linear-gradient(135deg,var(--accent),var(--purple));
  ...
  box-shadow:0 4px 16px rgba(79,142,247,.4);animation:pulse 2s infinite;cursor:pointer" onclick="toggleChat()">
  Besoin d'aide ? 💬</div>
...
<div id="chatbot-widget" style="display:none;position:fixed;bottom:90px;right:24px;width:360px;
  ...
  <div style="padding:12px 16px;border-top:1px solid var(--border);display:flex;gap:8px">
    ...
    <button onclick="sendMsg()" style="background:linear-gradient(135deg,var(--accent),var(--purple));border:none;
      border-radius:10px;width:40px;height:40px;cursor:pointer;font-size:18px">➤</button>
  </div>
</div>
...
.typing{{display:flex;gap:4px;align-items:center;padding:12px 16px;background:var(--card2);
  border-radius:12px;width:60px}}
.typing span{{width:6px;height:6px;background:var(--text2);border-radius:50%;animation:blink 1s infinite}}
...
async function sendMsg(){{
  ...
  try{{
    var r = await fetch('/api/chatbot', {{method:'POST',headers:{{'Content-Type':'application/json'}},
      body:JSON.stringify({{message:msg}})}});
    var d = await r.json();
    typing.remove();
    addMsg(d.response || "Désolé, je n'ai pas compris. Rejoins notre Discord pour plus d'aide !", 'bot');
  }}catch(e){{
    typing.remove();
    addMsg("Oups, une erreur s'est produite. Réessaie ou contacte-nous sur Discord !", 'bot');
  }}
}}
...
setTimeout(function(){{
  if(!notifHidden) document.getElementById('chatbot-notif').style.display='none';
}}, 8000);
</script>"""

    return render_template_string(f"""<!DOCTYPE html><html lang="fr"><head>
<meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>{site_title} — {site_tagline}</title>{SITE_CSS}</head><body>
{announce_html}
{_site_nav("home")}
<section class="hero">
...
<div style="display:flex;gap:0;border-bottom:1px solid var(--border);margin-bottom:20px;overflow-x:auto">
  {''.join(f'<a href="/admin/site?tab={t}" style="padding:10px 18px;font-size:13px;cursor:pointer;white-space:nowrap;{tab_style(t)}">{label}</a>'
    for t,label in [("overview","📊 Vue d'ensemble"),("content","✏️ Contenu"),("products","📦 Produits"),
                    ...
                    ("reviews","⭐ Avis"),("chatbot","🤖 Chatbot"),("stats","📈 Stats")])}
</div>"""
