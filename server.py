...
def init_db():
    ...
    # Table commandes avec factures
    c.execute("""CREATE TABLE IF NOT EXISTS orders (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        order_id TEXT UNIQUE,
        discord_id TEXT, discord_tag TEXT,
        plan TEXT, price REAL, paypal_name TEXT, amount_paid TEXT,
    ...
    # Table logs revendeurs
    c.execute("""CREATE TABLE IF NOT EXISTS reseller_key_logs (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        discord_id TEXT, discord_tag TEXT,
        plan TEXT, qty INTEGER, keys TEXT,
        cost REAL, ts INTEGER
    )""")
    # Table logs bot
    ...
    # Migration: must_change_pass pour resellers
    try: c.execute("ALTER TABLE admins ADD COLUMN must_change_pass INTEGER DEFAULT 0")
    except: pass
    try: c.execute("ALTER TABLE admins ADD COLUMN temp_password TEXT")
    except: pass
    try: c.execute("ALTER TABLE admins ADD COLUMN discord_id TEXT")
    except: pass
...
def admin_required(f):
    ...
    def decorated(*args, **kwargs):
        ...
        # Forcer changement mot de passe si provisoire
        if request.endpoint not in ("admin_change_password", "admin_logout"):
            conn = get_db()
            adm = conn.execute(
                "SELECT must_change_pass FROM admins WHERE username=? AND active=1",
                (session.get("admin_user",""),)
            ).fetchone()
            conn.close()
            if adm and adm["must_change_pass"] == 1:
...
def api_support_request():
    ...
    with _remote_lock:
        _remote_sessions[session_id] = {
            ...
            "frame_quality": 85,  # qualité JPEG demandée au client
            "frame_interval": 16, # ms entre chaque frame (60fps)
...
def api_support_frame():
    ...
    with _remote_lock:
        ...
        interval = sess.get("frame_interval", 16)
    ...
    if request.method == "POST":
        ...
        if row and row["password_hash"] == _hash_password(p):
            ...
            if role == "reseller":
                ...
                session["admin_role"]      = "reseller"
                add_log("OK","RESELLER",f"Login reseller via admin: {u}")
                # Si mot de passe provisoire
                if row.get("must_change_pass") == 1:
                    return redirect("/admin/change-password")
            ...
            # Si mot de passe provisoire → forcer changement
            if row.get("must_change_pass") == 1:
                return redirect("/admin/change-password")
...
def admin_remote_quality(session_id):
    ...
    with _remote_lock:
        ...
        if sess:
            ...
            sess["frame_interval"] = max(16, min(500, interval))
    return jsonify({"success": True})
...
def admin_create_admin():
    ...
    # Normaliser les anciens noms de rôles
    legacy = {"moderator": "moderateur", "staff": "moderateur", "admin": "manager"}
    r = legacy.get(r, r)
    if r not in ("moderateur", "manager", "owner"): r = "moderateur"
    if u and p:
        conn = get_db()
        try:
...
def _nav(active):
    ...
    items = [
        ...
        ("/admin/logs-revendeurs","🔑","Logs Revendeurs","manager"),
        ("/admin/logs-bot","🤖","Logs Bot","manager"),
        ("/admin/logs","📋","Logs",""),
    ...
    for item in items:
        ...
        # Filtrer les liens restreints selon le rôle (4ème champ = grade requis)
        if cnt in ("manager", "owner"):
            from flask import session as _sess
            _role = _sess.get("admin_role","")
            if _role not in ("owner","manager"):
                continue
            cnt = ""  # pas de badge pour les items de grade
        cls = "active" if active == href else ""
        badge = f'<span class="cnt {"yellow" if cnt and cnt.isdigit() and int(cnt)>0 and href not in ["/admin/remote"] else "green" if href=="/admin/remote" else ""}">{cnt}</span>' if cnt and cnt.isdigit() else ""
...
# === HARDWARE PANEL BUILDERS ===================================================

def _hw_row(label, value, color="var(--cyan)"):
    if not value or str(value) in ("", "—", "N/A", "Non renseigné"):
        val_html = '<span style="color:var(--muted);font-size:12px">Non renseigné</span>'
    else:
        val_html = f'<span style="color:{color};font-weight:600;font-size:13px">{value}</span>'
    return (
        '<div style="display:flex;justify-content:space-between;align-items:center;'
        'padding:9px 0;border-bottom:1px solid var(--border)">'
        f'<span style="color:var(--text2);font-size:12px;min-width:120px">{label}</span>'
        f'{val_html}</div>'
    )

def _hw_card(title, icon, rows_html):
    return (
        '<div style="background:#0d0d1a;border:1px solid var(--border);'
        'border-radius:10px;padding:14px;margin-bottom:12px">'
        f'<div style="font-size:11px;color:var(--text2);font-weight:700;text-transform:uppercase;'
        f'letter-spacing:.8px;margin-bottom:10px">{icon} {title}</div>'
        f'{rows_html}</div>'
    )

def _parse_cpu(cpu_raw):
    if not cpu_raw: return {}
    if "|" in cpu_raw:
        p = cpu_raw.split("|")
        return {"model": p[0].strip(), "cores": p[1].strip() if len(p)>1 else "—",
                "threads": p[2].strip() if len(p)>2 else "—",
                "freq": (p[3].strip()+" MHz") if len(p)>3 else "—",
                "temp": (p[4].strip()+"°C") if len(p)>4 and p[4].strip() not in ("","0","N/A") else "N/A"}
    try:
        import json; d = json.loads(cpu_raw)
        return {"model": d.get("model",cpu_raw), "cores": str(d.get("cores","—")),
                "threads": str(d.get("threads","—")), "freq": str(d.get("freq","—")),
                "temp": str(d.get("temp","N/A"))}
    except: pass
    return {"model": cpu_raw, "cores": "—", "threads": "—", "freq": "—", "temp": "N/A"}

def _parse_gpu(gpu_raw):
    if not gpu_raw: return {}
    if "|" in gpu_raw:
        p = gpu_raw.split("|")
        return {"model": p[0].strip(), "vram": p[1].strip() if len(p)>1 else "—",
                "driver": p[2].strip() if len(p)>2 else "—"}
    try:
        import json; d = json.loads(gpu_raw)
        return {"model": d.get("model",gpu_raw), "vram": str(d.get("vram","—")),
                "driver": str(d.get("driver","—"))}
    except: pass
    return {"model": gpu_raw, "vram": "—", "driver": "—"}

def _parse_ram(ram_raw):
    if not ram_raw: return {}
    if "|" in ram_raw:
        p = ram_raw.split("|")
        return {"total": p[0].strip(), "used": p[1].strip() if len(p)>1 else "—",
                "speed": p[2].strip() if len(p)>2 else "—",
                "vendor": p[3].strip() if len(p)>3 else "—"}
    try:
        import json; d = json.loads(ram_raw)
        return {"total": str(d.get("total","—")), "used": str(d.get("used","—")),
                "speed": str(d.get("speed","—")), "vendor": str(d.get("vendor","—"))}
    except: pass
    return {"total": ram_raw, "used": "—", "speed": "—", "vendor": "—"}

def _parse_mb(mb_raw):
    if not mb_raw: return {}
    if "|" in mb_raw:
        p = mb_raw.split("|")
        return {"vendor": p[0].strip(), "model": p[1].strip() if len(p)>1 else "—"}
    try:
        import json; d = json.loads(mb_raw)
        return {"vendor": d.get("vendor","—"), "model": d.get("model",mb_raw)}
    except: pass
    return {"vendor": mb_raw, "model": "—"}

def _parse_disk(disk_raw):
    if not disk_raw: return []
    try:
        import json; d = json.loads(disk_raw)
        return d if isinstance(d, list) else [d]
    except: pass
    if "|" in disk_raw:
        p = disk_raw.split("|"); disks = []
        for i in range(0, len(p)-1, 2):
            disks.append({"name": p[i].strip(), "size": p[i+1].strip() if i+1 < len(p) else "—"})
        return disks or [{"name": disk_raw, "size": "—"}]
    return [{"name": disk_raw, "size": "—"}]

def _parse_os(os_raw):
    if not os_raw: return {}
    if "|" in os_raw:
        p = os_raw.split("|")
        return {"name": p[0].strip(), "version": p[1].strip() if len(p)>1 else "—",
                "arch": p[2].strip() if len(p)>2 else "—",
                "lang": p[3].strip() if len(p)>3 else "—"}
    try:
        import json; d = json.loads(os_raw)
        return {"name": d.get("name",os_raw), "version": str(d.get("version","—")),
                "arch": str(d.get("arch","—")), "lang": str(d.get("lang","—"))}
    except: pass
    return {"name": os_raw, "version": "—", "arch": "—", "lang": "—"}

def _build_hw_general(u):
    cpu_d = _parse_cpu(u.get("cpu_info","") or "")
    gpu_d = _parse_gpu(u.get("gpu_info","") or "")
    ram_d = _parse_ram(u.get("ram_info","") or "")
    mb_d  = _parse_mb(u.get("motherboard_info","") or "")
    os_d  = _parse_os(u.get("os_info","") or "")

    cores_txt = ("—" if cpu_d.get("cores","") in ("","—")
                 else cpu_d.get("cores","—") + " physiques · " + cpu_d.get("threads","—") + " logiques")
    cpu_rows = (_hw_row("Modèle", cpu_d.get("model","")) +
                _hw_row("Cœurs", cores_txt) +
                _hw_row("Fréquence", cpu_d.get("freq","")) +
                _hw_row("Température", cpu_d.get("temp","N/A")))
    gpu_rows = (_hw_row("Modèle", gpu_d.get("model","")) +
                _hw_row("VRAM", gpu_d.get("vram","")) +
                _hw_row("Driver", gpu_d.get("driver","")))
    ram_rows = (_hw_row("Total", ram_d.get("total","")) +
                _hw_row("Utilisée", ram_d.get("used","")) +
                _hw_row("Vitesse", ram_d.get("speed","")) +
                _hw_row("Fabricant", ram_d.get("vendor","")))
    mb_rows  = (_hw_row("Fabricant", mb_d.get("vendor","")) +
                _hw_row("Modèle", mb_d.get("model","")))
    os_rows  = (_hw_row("Système", os_d.get("name","")) +
                _hw_row("Version", os_d.get("version","")) +
                _hw_row("Architecture", os_d.get("arch","")) +
                _hw_row("Langue", os_d.get("lang","")))

    grid = (
        '<div style="display:grid;grid-template-columns:1fr 1fr;gap:10px">'
        + _hw_card("PROCESSEUR", "🖥", cpu_rows)
        + _hw_card("CARTE GRAPHIQUE", "🎮", gpu_rows)
        + _hw_card("MÉMOIRE RAM", "💾", ram_rows)
        + _hw_card("CARTE MÈRE", "🔌", mb_rows)
        + "</div>"
    )
    os_section = _hw_card("SYSTÈME D'EXPLOITATION", "💻", os_rows) if os_d.get("name") else ""
    return grid + os_section

def _build_hw_storage(u):
    disks = _parse_disk(u.get("disk_info","") or "")
    if not disks or (len(disks)==1 and disks[0].get("name","") in ("","—")):
        return '<div style="color:var(--muted);text-align:center;padding:24px;font-size:13px">Aucune donnée de stockage disponible</div>'
    result = ""
    for i, d in enumerate(disks):
        name  = d.get("name","Disque "+str(i+1))
        size  = d.get("size","—"); free = d.get("free","—"); used = d.get("used","—")
        dtype = d.get("type","")
        icon  = "💿" if "HDD" in str(dtype).upper() else "🔷"
        result += (
            '<div style="background:#0d0d1a;border:1px solid var(--border);'
            'border-radius:10px;padding:14px;margin-bottom:10px">'
            f'<div style="font-size:12px;font-weight:700;color:var(--cyan);margin-bottom:8px">{icon} {name}</div>'
            '<div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:8px">'
            f'<div><div style="font-size:10px;color:var(--muted);margin-bottom:2px">TOTAL</div>'
            f'<div style="color:var(--cyan);font-weight:600">{size}</div></div>'
            f'<div><div style="font-size:10px;color:var(--muted);margin-bottom:2px">UTILISÉ</div>'
            f'<div style="color:var(--yellow);font-weight:600">{used}</div></div>'
            f'<div><div style="font-size:10px;color:var(--muted);margin-bottom:2px">LIBRE</div>'
            f'<div style="color:var(--green);font-weight:600">{free}</div></div>'
            '</div></div>'
        )
    return result

def _build_hw_network(u, geo):
    ip      = u.get("ip","") or "—"
    flag    = geo.get("flag","🌐"); country = geo.get("country","?"); city = geo.get("city","?")
    isp     = geo.get("isp","?"); proxy = geo.get("proxy", False)
    hwid    = u.get("hwid","") or ""
    proxy_badge = (
        '<span style="background:var(--yellow);color:#000;font-size:10px;padding:2px 8px;border-radius:4px;font-weight:700">⚠️ VPN/Proxy</span>'
        if proxy else
        '<span style="background:var(--green);color:#000;font-size:10px;padding:2px 8px;border-radius:4px;font-weight:700">✅ IP directe</span>'
    )
    return (
        '<div style="background:#0d0d1a;border:1px solid var(--border);border-radius:10px;padding:14px;margin-bottom:12px">'
        '<div style="font-size:11px;color:var(--text2);font-weight:700;text-transform:uppercase;letter-spacing:.8px;margin-bottom:10px">🌐 CONNEXION</div>'
        '<div style="display:flex;align-items:center;gap:12px;margin-bottom:12px">'
        f'<div style="font-size:32px">{flag}</div>'
        '<div>'
        f'<div style="color:var(--cyan);font-weight:700;font-size:15px">{ip}</div>'
        f'<div style="color:var(--text2);font-size:12px">{city}, {country}</div>'
        f'<div style="color:var(--muted);font-size:11px">ISP: {isp}</div>'
        '</div>'
        f'<div style="margin-left:auto">{proxy_badge}</div>'
        '</div>'
        '<div style="padding:8px 0;border-top:1px solid var(--border)">'
        '<div style="font-size:10px;color:var(--muted);margin-bottom:4px">HWID</div>'
        f'<div style="font-family:monospace;font-size:11px;color:var(--text2);word-break:break-all">{hwid or "Non enregistré"}</div>'
        '</div></div>'
    )

def _build_hw_divers(u):
    rows = (
        _hw_row("Nom d'utilisateur", u.get("username","—"), "var(--text)") +
        _hw_row("Discord ID", u.get("discord_id","—") or "—", "var(--text)") +
        _hw_row("Connexions totales", str(u.get("connections",0) or 0), "var(--yellow)") +
        _hw_row("1ère connexion faite", ("✅ Oui" if u.get("first_login_done") else "❌ Non"), "var(--green)") +
        _hw_row("Créé le", fmt_ts(u.get("created_at")) if u.get("created_at") else "—", "var(--text2)") +
        _hw_row("Dernier login", fmt_ts(u.get("last_login")) if u.get("last_login") else "—", "var(--text2)")
    )
    return _hw_card("INFORMATIONS COMPTE", "📋", rows)

# === END HARDWARE PANEL BUILDERS ===============================================


def _page_user_detail(user, logs, alerts, login_hist, notes, remote_hist, geo, msg, admin_user, admin_role):
    u = dict(user)  # Convert sqlite3.Row to dict
    # hw_html via _build_hw_* functions
...
<div class="card">
  ...
  <div style="display:flex;gap:2px;margin-bottom:14px;border-bottom:1px solid var(--border);padding-bottom:0">
    <button onclick="showHwTab('general',this)" id="hw-tab-general"
      style="padding:6px 14px;background:transparent;border:none;border-bottom:2px solid var(--cyan);color:var(--cyan);font-size:12px;font-weight:700;cursor:pointer;letter-spacing:.5px">GÉNÉRAL</button>
    <button onclick="showHwTab('stockage',this)" id="hw-tab-stockage"
      style="padding:6px 14px;background:transparent;border:none;border-bottom:2px solid transparent;color:var(--text2);font-size:12px;font-weight:700;cursor:pointer;letter-spacing:.5px">STOCKAGE</button>
    <button onclick="showHwTab('reseau',this)" id="hw-tab-reseau"
      style="padding:6px 14px;background:transparent;border:none;border-bottom:2px solid transparent;color:var(--text2);font-size:12px;font-weight:700;cursor:pointer;letter-spacing:.5px">RÉSEAU</button>
    <button onclick="showHwTab('divers',this)" id="hw-tab-divers"
      style="padding:6px 14px;background:transparent;border:none;border-bottom:2px solid transparent;color:var(--text2);font-size:12px;font-weight:700;cursor:pointer;letter-spacing:.5px">DIVERS</button>
  </div>

  <!-- Onglet GÉNÉRAL -->
  <div id="hw-panel-general">
    {_build_hw_general(u)}
  </div>

  <!-- Onglet STOCKAGE -->
  <div id="hw-panel-stockage" style="display:none">
    {_build_hw_storage(u)}
  </div>

  <!-- Onglet RÉSEAU -->
  <div id="hw-panel-reseau" style="display:none">
    {_build_hw_network(u, geo)}
  </div>

  <!-- Onglet DIVERS -->
  <div id="hw-panel-divers" style="display:none">
    {_build_hw_divers(u)}
  </div>

  <script>
  function showHwTab(name, btn) {{
    ['general','stockage','reseau','divers'].forEach(function(t) {{
      document.getElementById('hw-panel-'+t).style.display = t===name ? '' : 'none';
      var b = document.getElementById('hw-tab-'+t);
      b.style.borderBottomColor = t===name ? 'var(--cyan)' : 'transparent';
      b.style.color = t===name ? 'var(--cyan)' : 'var(--text2)';
    }});
  }}
  </script>
...
<div>
  ...
  <div class="card">
    ...
    <div style="margin-top:10px;padding:10px;background:rgba(0,255,136,.06);border-radius:8px;border:1px solid rgba(0,255,136,.15);font-size:11px;color:var(--text2)">
      ⚡ HTTP polling ~16ms (60fps) · JPEG quality 85<br>
...
def _page_owners(admins, admin_user, admin_role):
    rows = ""
    for a in admins:
        role_label = {"owner":"👑 Owner","manager":"🔧 Manager","moderateur":"🛡 Modérateur","moderator":"🛡 Modérateur","reseller":"🤝 Revendeur","staff":"🛡 Modérateur","admin":"🔧 Manager"}.get(a["role"] or "", a["role"] or "?")
        role_color = {"owner":"b-yellow","manager":"b-green","moderateur":"b-cyan","moderator":"b-cyan","reseller":"b-purple","staff":"b-cyan","admin":"b-green"}.get(a["role"] or "","b-gray")
        rb = f'<span class="badge {role_color}">{role_label}</span>'
...
<div class="card">
  ...
  <form method="POST" action="/admin/owners/create">
    <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(160px,1fr));gap:10px;align-items:end">
      ...
      <div class="form-group" style="margin:0"><label>Rôle</label><select name="role"><option value="moderateur">🛡 Modérateur</option><option value="manager">🔧 Manager</option><option value="owner">👑 Owner</option></select></div>
...
def init_shop_db():
    ...
    if not conn.execute("SELECT 1 FROM site_products").fetchone():
        conn.execute("INSERT INTO site_products (plan,name,price,description,features,badge,active,sort_order) VALUES (?,?,?,?,?,?,?,?)",
            ("NORMAL","NORMAL",20.0,"Toutes les optimisations essentielles pour booster ton PC gaming.",
             "+50 optimisations|Gaming Pack 25+ jeux|Réseau & confidentialité|Espace client inclus",
             "","1",1))
        conn.execute("INSERT INTO site_products (plan,name,price,description,features,badge,active,sort_order) VALUES (?,?,?,?,?,?,?,?)",
            ("PRO","PRO",40.0,"L'expérience gaming ultime avec toutes les optimisations avancées.",
...
# ─── /api/internal/create_order  (nouveau, depuis bot paiement v2) ─────────────
@app.route("/api/internal/create_order", methods=["POST"])
def api_create_order_v2():
    data = request.json or {}
    if data.get("bot_token") != BOT_TOKEN:
        return jsonify({"success": False, "error": "Unauthorized"}), 403
    discord_id  = str(data.get("discord_id", "")).strip()
    discord_tag = str(data.get("discord_tag", "")).strip()
    plan        = (data.get("plan", "") or "").upper()
    price       = float(data.get("price", 0))
    paypal_name = str(data.get("paypal_name", "")).strip()
    amount_paid = str(data.get("amount_paid", "")).strip()
    ...
    order_id = "XW-" + "".join(random.choices(string.ascii_uppercase + string.digits, k=8))
    conn = get_db()
    try:
        conn.execute(
            "INSERT INTO orders (order_id,discord_id,discord_tag,plan,price,paypal_name,amount_paid,status,created_at) VALUES (?,?,?,?,?,?,?,?,?)",
            (order_id, discord_id, discord_tag, plan, price, paypal_name, amount_paid, "pending", int(_time.time()))
        )
        conn.commit()
    except Exception as e:
        conn.rollback(); conn.close()
        return jsonify({"success": False, "error": str(e)})
    conn.close()
    add_log("OK", "SHOP", f"Commande {order_id} créée — {plan} {price}€ discord={discord_id} pp={paypal_name}", "bot")
    return jsonify({"success": True, "order_id": order_id})


# ─── /api/internal/complete_order_v2  (approuver commande + générer facture PDF) ──
@app.route("/api/internal/complete_order_v2", methods=["POST"])
def api_complete_order_v2():
    data = request.json or {}
    if data.get("bot_token") != BOT_TOKEN:
        return jsonify({"success": False, "error": "Unauthorized"}), 403
    order_id   = str(data.get("order_id", "")).upper().strip()
    discord_id = str(data.get("discord_id", "")).strip()
    admin_by   = data.get("admin", "bot")
    ...
    plan  = order["plan"]
    price = order["price"]
    paypal_name = order["paypal_name"] or ""

    # Générer une clé de licence
    license_key = generate_key(plan)
    conn.execute("INSERT INTO licenses (key,plan,status,created_at) VALUES (?,?,?,?)",
                 (license_key, plan, "sold", int(_time.time())))

    # Mettre à jour la commande
    conn.execute(
        "UPDATE orders SET status='completed', license_key=?, confirmed_at=?, confirmed_by=? WHERE order_id=?",
        (license_key, int(_time.time()), admin_by, order_id)
    )
    conn.commit()
    conn.close()
    add_log("OK", "SHOP", f"Commande {order_id} approuvée par {admin_by}", admin_by)
    return jsonify({
        "success": True,
        "order_id": order_id,
        "plan": plan,
...
# ─── /api/internal/bot_log  ────────────────────────────────────────────────────
@app.route("/api/internal/bot_log", methods=["POST"])
def api_bot_log():
    data = request.json or {}
    if data.get("bot_token") != BOT_TOKEN:
    ...
    conn = get_db()
    conn.execute("INSERT INTO bot_logs (ts,level,msg) VALUES (?,?,?)", (int(_time.time()), level, msg))
    conn.commit(); conn.close()
    return jsonify({"success": True})


# ─── /api/internal/log_reseller_gen  ──────────────────────────────────────────
@app.route("/api/internal/log_reseller_gen", methods=["POST"])
def api_log_reseller_gen():
    data = request.json or {}
    if data.get("bot_token") != BOT_TOKEN:
        return jsonify({"success": False}), 403
    conn = get_db()
    conn.execute(
        "INSERT INTO reseller_key_logs (discord_id,discord_tag,plan,qty,keys,cost,ts) VALUES (?,?,?,?,?,?,?)",
        (
            str(data.get("discord_id","")),
            str(data.get("discord_tag","")),
            str(data.get("plan","")),
            int(data.get("qty",0)),
...
# ─── /api/internal/create_staff_account  ──────────────────────────────────────
@app.route("/api/internal/create_staff_account", methods=["POST"])
def api_create_staff_account():
    data = request.json or {}
    if data.get("bot_token") != BOT_TOKEN:
        return jsonify({"success": False, "error": "Unauthorized"}), 403
    username   = str(data.get("username","")).strip().lower()
    grade      = str(data.get("grade","")).strip().lower()   # moderateur | manager | owner
    created_by = str(data.get("created_by","bot"))
    discord_id = str(data.get("discord_id","")).strip()

    # Normaliser les anciens grades legacy → nouveaux grades
    legacy_map = {"staff": "moderateur", "admin": "manager", "moderator": "moderateur",
                  "mod": "moderateur", "modérateur": "moderateur"}
    grade = legacy_map.get(grade, grade)
    grade_map = {"moderateur": "moderateur", "manager": "manager", "owner": "owner"}
    if grade not in grade_map:
        return jsonify({"success": False, "error": "Grade invalide — choisis : moderateur, manager ou owner"})
    if not username:
        return jsonify({"success": False, "error": "Username manquant"})

    import random, string
    temp_pass = "Xywez-" + "".join(random.choices(string.digits + string.ascii_letters, k=8))
    pw_hash   = _hash_password(temp_pass)

    conn = get_db()
    existing = conn.execute("SELECT 1 FROM admins WHERE username=?", (username,)).fetchone()
    if existing:
        conn.close()
        return jsonify({"success": False, "error": "Ce username existe déjà"})
    try:
        conn.execute(
            "INSERT INTO admins (username,password_hash,role,created_at,created_by,discord_id,active,must_change_pass,temp_password) VALUES (?,?,?,?,?,?,1,1,?)",
            (username, pw_hash, grade, int(_time.time()), created_by, discord_id, temp_pass)
        )
        conn.commit()
    except Exception as e:
        conn.rollback(); conn.close()
        return jsonify({"success": False, "error": str(e)})
    conn.close()
    add_log("OK", "ADMIN", f"Compte staff {username} ({grade}) créé par {created_by}", created_by)
    return jsonify({"success": True, "temp_password": temp_pass, "username": username})


# ─── /api/internal/check_promo  ────────────────────────────────────────────────
@app.route("/api/internal/check_promo")
def api_internal_check_promo():
    if request.args.get("bot_token") != BOT_TOKEN:
        return jsonify({"valid": False, "error": "Unauthorized"}), 403
    code = (request.args.get("code","") or "").upper().strip()
    conn = get_db()
    row = conn.execute(
        "SELECT * FROM promo_codes WHERE code=? AND (max_uses=0 OR uses_count<max_uses)",
        (code,)
    ).fetchone()
    conn.close()
    if not row:
        return jsonify({"valid": False, "error": "Code invalide ou expiré"})
    return jsonify({"valid": True, "discount": row["discount"], "code": code})
...
def admin_commandes():
    ...
    for o in orders:
        status_badge = {
            "pending":   '<span style="color:#ffcc00;background:#2a2000;padding:2px 8px;border-radius:6px;font-size:12px">⏳ En attente</span>',
            "completed": '<span style="color:#00ff88;background:#002a14;padding:2px 8px;border-radius:6px;font-size:12px">✅ Complété</span>',
            "cancelled": '<span style="color:#ff4466;background:#2a0010;padding:2px 8px;border-radius:6px;font-size:12px">❌ Annulé</span>',
        }.get(o["status"] or "pending", o["status"])
        from datetime import datetime as _dt
        ts = _dt.fromtimestamp(o["created_at"]).strftime("%d/%m %H:%M") if o["created_at"] else "—"
        conf = _dt.fromtimestamp(o["confirmed_at"]).strftime("%d/%m %H:%M") if o.get("confirmed_at") else "—"
        rows_html += f"""
        <tr>
          <td><code style="color:#00e5ff">{o['order_id'] or '—'}</code></td>
          <td>{o['discord_tag'] or o['discord_id'] or '—'}</td>
          <td>{'⭐ PRO' if o['plan']=='PRO' else '🔷 NORMAL'}</td>
          <td>{o['price']:.2f}€</td>
          <td>{o['paypal_name'] or '—'}</td>
          <td>{o['amount_paid'] or '—'}€</td>
          <td>{status_badge}</td>
          <td><code>{o['license_key'] or '—'}</code></td>
          <td style="font-size:11px;color:#888">{ts}</td>
          <td style="font-size:11px;color:#888">{o['confirmed_by'] or '—'} · {conf}</td>
        </tr>"""

    # Barre de recherche
    search_form = """
    <form method="GET" action="/admin/commandes/search" style="display:flex;gap:10px;margin-bottom:20px">
      <input name="q" placeholder="Discord ID, PayPal, N° commande..." style="flex:1;padding:8px 12px;background:#1a1a2e;border:1px solid #333;border-radius:8px;color:#fff">
      <button type="submit" style="padding:8px 20px;background:#4f8ef7;border:none;border-radius:8px;color:#fff;cursor:pointer">🔍 Rechercher</button>
    </form>"""

    content = f"""
    <div style="padding:24px">
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:20px">
        <h2 style="color:#fff;margin:0">🛒 Commandes</h2>
        <span style="color:#888;font-size:13px">{len(orders)} commandes</span>
      </div>
      {search_form}
      <div style="overflow-x:auto">
        <table style="width:100%;border-collapse:collapse;font-size:13px">
          <thead>
            <tr style="background:#1a1a2e;color:#888;text-align:left">
              <th style="padding:10px">N° Commande</th>
              <th style="padding:10px">Discord</th>
              <th style="padding:10px">Plan</th>
              <th style="padding:10px">Prix</th>
              <th style="padding:10px">Prénom PP</th>
              <th style="padding:10px">Montant déclaré</th>
              <th style="padding:10px">Statut</th>
              <th style="padding:10px">Clé</th>
              <th style="padding:10px">Créée</th>
              <th style="padding:10px">Confirmée par</th>
            </tr>
          </thead>
          <tbody>
            {rows_html or '<tr><td colspan="10" style="text-align:center;color:#888;padding:20px">Aucune commande</td></tr>'}
          </tbody>
        </table>
      </div>
    </div>"""
    return _layout("Commandes", "/admin/commandes", content, session["admin_user"], session["admin_role"])


@app.route("/admin/commandes/search")
@admin_required
def admin_commandes_search():
    q = (request.args.get("q","") or "").strip()
    conn = get_db()
    orders = conn.execute(
        "SELECT * FROM orders WHERE order_id LIKE ? OR discord_id LIKE ? OR discord_tag LIKE ? OR paypal_name LIKE ? ORDER BY created_at DESC LIMIT 100",
        (f"%{q}%", f"%{q}%", f"%{q}%", f"%{q}%")
    ).fetchall()
    conn.close()
    ...
    rows_html = ""
    for o in orders:
        status_badge = {"pending":"⏳ En attente","completed":"✅ Complété","cancelled":"❌ Annulé"}.get(o["status"] or "pending", o["status"])
        ts = _dt.fromtimestamp(o["created_at"]).strftime("%d/%m %H:%M") if o["created_at"] else "—"
        rows_html += f"""<tr>
          <td><code style="color:#00e5ff">{o['order_id'] or '—'}</code></td>
          <td>{o['discord_tag'] or o['discord_id'] or '—'}</td>
    ...
    content = f"""
    <div style="padding:24px">
      <h2 style="color:#fff">🔍 Résultats pour : <span style="color:#4f8ef7">{q}</span></h2>
      <a href="/admin/commandes" style="color:#888;font-size:13px">← Retour aux commandes</a>
      <div style="overflow-x:auto;margin-top:16px">
        <table style="width:100%;border-collapse:collapse;font-size:13px">
          <thead><tr style="background:#1a1a2e;color:#888">
            <th style="padding:10px">N° Commande</th><th style="padding:10px">Discord</th>
            <th style="padding:10px">Plan</th><th style="padding:10px">Prix</th>
            <th style="padding:10px">Prénom PP</th><th style="padding:10px">Statut</th>
            <th style="padding:10px">Clé</th><th style="padding:10px">Date</th>
          </tr></thead>
          <tbody>
            {rows_html or '<tr><td colspan="8" style="text-align:center;color:#888;padding:20px">Aucun résultat</td></tr>'}
          </tbody>
        </table>
      </div>
    </div>"""
    return _layout("Recherche commandes", "/admin/commandes", content, session["admin_user"], session["admin_role"])


# ─── /admin/logs-revendeurs  ───────────────────────────────────────────────────
@app.route("/admin/logs-revendeurs")
@admin_required
def admin_logs_revendeurs():
    role = session.get("admin_role","")
    if role not in ("owner","manager"):
        return "Accès refusé", 403
    conn = get_db()
    logs = conn.execute(
        "SELECT * FROM reseller_key_logs ORDER BY ts DESC LIMIT 300"
    ).fetchall()
    conn.close()
    ...
    rows_html = ""
    for lg in logs:
        ts = _dt.fromtimestamp(lg["ts"]).strftime("%d/%m/%Y %H:%M") if lg["ts"] else "—"
        keys_list = ", ".join(f"<code>{k}</code>" for k in (json.loads(lg["keys"]) if lg["keys"] else []))
        rows_html += f"""<tr>
          <td style="font-size:11px">{ts}</td>
          <td>{lg['discord_tag'] or lg['discord_id'] or '—'}</td>
          <td>{'⭐ PRO' if lg['plan']=='PRO' else ('🔷 NORMAL' if lg['plan']=='NORMAL' else lg['plan'])}</td>
          <td style="text-align:center">{lg['qty']}</td>
          <td style="font-size:11px">{keys_list[:200] or '—'}</td>
          <td>{lg['cost']:.2f}€</td>
        </tr>"""

    content = f"""
    <div style="padding:24px">
      <h2 style="color:#fff;margin-bottom:20px">🔑 Logs Revendeurs — Clés générées</h2>
      <div style="overflow-x:auto">
        <table style="width:100%;border-collapse:collapse;font-size:13px">
          <thead><tr style="background:#1a1a2e;color:#888">
            <th style="padding:10px">Date</th><th style="padding:10px">Revendeur</th>
            <th style="padding:10px">Plan</th><th style="padding:10px">Qté</th>
            <th style="padding:10px">Clés générées</th><th style="padding:10px">Coût</th>
          </tr></thead>
          <tbody>
            {rows_html or '<tr><td colspan="6" style="text-align:center;color:#888;padding:20px">Aucun log</td></tr>'}
          </tbody>
        </table>
      </div>
    </div>"""
    return _layout("Logs Revendeurs", "/admin/logs-revendeurs", content, session["admin_user"], session["admin_role"])


# ─── /admin/logs-bot  ──────────────────────────────────────────────────────────
@app.route("/admin/logs-bot")
@admin_required
def admin_logs_bot():
    role = session.get("admin_role","")
    if role not in ("owner","manager"):
        return "Accès refusé", 403
    conn = get_db()
    logs = conn.execute("SELECT * FROM bot_logs ORDER BY ts DESC LIMIT 500").fetchall()
    conn.close()

    from datetime import datetime as _dt
    rows_html = ""
    for lg in logs:
        ts = _dt.fromtimestamp(lg["ts"]).strftime("%d/%m/%Y %H:%M:%S") if lg["ts"] else "—"
        color = "#ff4466" if lg["level"] in ("ERROR","WARN") else ("#ffcc00" if lg["level"]=="WARN" else "#aaa")
        rows_html += f"""<tr>
          <td style="font-size:11px;color:#888">{ts}</td>
          <td><span style="color:{color};font-size:11px">{lg['level']}</span></td>
          <td style="font-size:12px;color:#ccc">{lg['msg']}</td>
        </tr>"""

    content = f"""
    <div style="padding:24px">
      <h2 style="color:#fff;margin-bottom:20px">🤖 Logs Bot Discord</h2>
      <div style="overflow-x:auto">
        <table style="width:100%;border-collapse:collapse;font-size:13px">
          <thead><tr style="background:#1a1a2e;color:#888">
          ...
          <tbody>
            {rows_html or '<tr><td colspan="3" style="text-align:center;color:#888;padding:20px">Aucun log</td></tr>'}
          </tbody>
        </table>
      </div>
    </div>"""
    return _layout("Logs Bot", "/admin/logs-bot", content, session["admin_user"], session["admin_role"])
...
def admin_change_password():
    ...
    if request.method == "POST":
        ...
        if len(new_pass) < 8:
            ...
            conn = get_db()
            conn.execute(
                "UPDATE admins SET password_hash=?, must_change_pass=0, temp_password=NULL WHERE username=?",
                (pw_hash, session["admin_user"])
            )
            conn.commit(); conn.close()
            success = "Mot de passe changé avec succès !"
            add_log("OK","ADMIN", f"Changement mdp par {session['admin_user']}", session["admin_user"])

    content = f"""
    <div style="max-width:440px;margin:60px auto;padding:32px;background:#1a1a2e;border-radius:16px;border:1px solid #333">
      <h2 style="color:#fff;margin-bottom:8px">🔑 Changer de mot de passe</h2>
      <p style="color:#888;margin-bottom:24px;font-size:14px">Tu dois définir un nouveau mot de passe pour continuer.</p>
      {'<div style="color:#ff4466;margin-bottom:12px">'+error+'</div>' if error else ''}
      {'<div style="color:#00ff88;margin-bottom:12px">'+success+'</div>' if success else ''}
      <form method="POST">
        <div style="margin-bottom:16px">
          <label style="color:#aaa;font-size:13px">Nouveau mot de passe</label>
          <input type="password" name="new_password" required style="width:100%;margin-top:6px;padding:10px;background:#0d0d1a;border:1px solid #333;border-radius:8px;color:#fff">
        </div>
        <div style="margin-bottom:20px">
          <label style="color:#aaa;font-size:13px">Confirmer le mot de passe</label>
          <input type="password" name="confirm_password" required style="width:100%;margin-top:6px;padding:10px;background:#0d0d1a;border:1px solid #333;border-radius:8px;color:#fff">
        </div>
        <button type="submit" style="width:100%;padding:12px;background:#4f8ef7;border:none;border-radius:8px;color:#fff;font-size:15px;cursor:pointer">✅ Confirmer</button>
      </form>
    </div>"""
    return _layout("Changer le mot de passe", "/admin/change-password", content, session["admin_user"], session["admin_role"])
...
# ─── /reseller_panel/keys-search  ─────────────────────────────────────────────
@app.route("/reseller_panel/keys")
@reseller_required
def reseller_panel_keys():
    r = _get_reseller()
    if not r:
        return redirect("/admin/login")
    conn = get_db()
    logs = conn.execute(
        "SELECT * FROM reseller_key_logs WHERE discord_id=? ORDER BY ts DESC LIMIT 200",
        (r["discord_id"],)
    ).fetchall()
    # Cherche aussi dans key_gen_log
    kgl = conn.execute(
        "SELECT * FROM key_gen_log WHERE reseller_id=? ORDER BY created_at DESC LIMIT 200",
        (r["discord_id"],)
    ).fetchall()
    conn.close()
    ...
    all_entries = []
    for lg in logs:
        keys = _json.loads(lg["keys"]) if lg["keys"] else []
        for k in keys:
            all_entries.append({
                "key": k,
                "plan": lg["plan"],
    ...
    if q:
        all_entries = [e for e in all_entries if q.lower() in (e.get("key","") or "").lower()]

    all_entries.sort(key=lambda x: x["ts"] or 0, reverse=True)

    for e in all_entries[:200]:
        ts = _dt.fromtimestamp(e["ts"]).strftime("%d/%m/%Y %H:%M") if e.get("ts") else "—"
        plan_b = "⭐ PRO" if e["plan"]=="PRO" else ("🔷 NORMAL" if e["plan"]=="NORMAL" else e["plan"])
        rows += f"""<tr>
          <td><code style="color:#00e5ff">{e['key']}</code></td>
          <td>{plan_b}</td>
          <td style="font-size:11px;color:#888">{ts}</td>
          <td style="font-size:11px;color:#666">{e.get('source','—')}</td>
          <td style="font-size:11px;color:#888">{e.get('user','—')}</td>
        </tr>"""

    content = f"""
    <div style="padding:24px;max-width:1000px;margin:0 auto">
      <h2 style="color:#fff;margin-bottom:16px">🔑 Mes clés générées ({len(all_entries)})</h2>
      <form method="GET" style="display:flex;gap:10px;margin-bottom:20px">
        <input name="q" value="{q}" placeholder="Rechercher une clé..." style="flex:1;padding:8px 12px;background:#1a1a2e;border:1px solid #333;border-radius:8px;color:#fff">
        <button type="submit" style="padding:8px 20px;background:#4f8ef7;border:none;border-radius:8px;color:#fff;cursor:pointer">🔍</button>
      </form>
      <div style="overflow-x:auto">
        <table style="width:100%;border-collapse:collapse;font-size:13px">
          <thead><tr style="background:#1a1a2e;color:#888;text-align:left">
            <th style="padding:10px">Clé</th>
            <th style="padding:10px">Plan</th>
            <th style="padding:10px">Date</th>
            <th style="padding:10px">Source</th>
            <th style="padding:10px">Utilisé par</th>
          </tr></thead>
          <tbody>
            {rows or '<tr><td colspan="5" style="text-align:center;color:#888;padding:20px">Aucune clé générée</td></tr>'}
          </tbody>
        </table>
      </div>
    </div>"""

    # Layout minimal panel revendeur
    return f"""<!DOCTYPE html><html lang="fr"><head><meta charset="UTF-8">
    <title>Mes Clés — Xywez Optimisation</title>
    <style>*{{box-sizing:border-box;margin:0;padding:0}}body{{background:#0d0d1a;color:#fff;font-family:'Segoe UI',sans-serif}}
    a{{color:#4f8ef7;text-decoration:none}}a:hover{{text-decoration:underline}}</style></head>
    <body>
    <div style="background:#1a1a2e;padding:14px 24px;display:flex;align-items:center;gap:16px;border-bottom:1px solid #333">
      <span style="color:#fff;font-weight:bold">🤝 Panel Revendeur</span>
      <a href="/reseller_panel" style="color:#888;font-size:13px">Dashboard</a>
      <a href="/reseller_panel/keys" style="color:#4f8ef7;font-size:13px">🔑 Mes Clés</a>
      <a href="/admin/logout" style="color:#ff4466;font-size:13px;margin-left:auto">Déconnexion</a>
    </div>
    {content}
    </body></html>"""
