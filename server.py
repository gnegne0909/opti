...
UPDATE_MAX_SIZE  = 200 * 1024 * 1024   # 200 Mo max par fichier
...
def _append_update_history(meta: dict):
    ...
    try:
        ...
        hist.insert(0, {k: meta[k] for k in ("version","filename","uploaded_at","uploaded_by","changelog","size_bytes") if k in meta})
...
def api_update_check():
    """
    Le client appelle cette route au démarrage.
    Payload : {"version": "7.0"}
    Réponse : {update_available, new_version, changelog, size_bytes, size_mb, download_url}
    """
    data           = request.get_json(silent=True) or {}
    ...
    # Comparaison sémantique : "7.0" < "7.1" < "8.0"
    def _parse(v):
        try:
    ...
    # Changelog structuré pour l'onglet MAJ du client
    raw_cl = meta.get("changelog", "")
    lines  = [l.strip() for l in raw_cl.splitlines() if l.strip()]
    changelog_entries = [
        [f"v{server_version}", "Nouveautés", lines[:20]]
    ] if lines and has_update else []
...
# ── API : téléchargement direct du .exe depuis le serveur ────────────────────
@app.route("/api/update/download")
def api_update_download():
    """
    Sert le fichier .exe directement depuis le dossier updates/.
    Supporte les requêtes Range pour la reprise de téléchargement.
    Compatible avec urllib.request.urlretrieve() côté client.
    """
    meta     = _get_update_meta()
    ...
    # Détermine l'extension pour le Content-Type
    ...
    if range_hdr:
        # Support Range: bytes=X-Y pour la reprise
        try:
            ...
            return Response(generate_range(filepath, start, end),
    ...
    return Response(generate_full(filepath), status=200,
                    headers=headers, direct_passthrough=True)

# ── API : upload .exe depuis l'admin (multipart) ─────────────────────────────
@app.route("/admin/update/upload_file", methods=["POST"])
@admin_required
def admin_update_upload_file():
    """
    Upload AJAX du fichier .exe / .py depuis le panel admin.
    Retourne JSON : {success, msg, filename, size_bytes}
    """
    ...
    old_meta = _get_update_meta()
    if old_meta.get("filename") and old_meta["filename"] != safe_name:
        old_path = os.path.join(UPDATE_DIR, old_meta["filename"])
        try:
    ...
    add_log("OK", "UPDATE",
            f"Upload v{version} par {session['admin_user']} — {safe_name} ({size_mb} Mo) SHA256:{sha256[:16]}…",
            session["admin_user"])
    _push_sse({"type": "update_pushed", "version": version, "admin": session["admin_user"], "size_mb": size_mb})

    return jsonify({
        "success":    True,
        "msg":        f"✅ v{version} uploadée avec succès ({size_mb} Mo)",
        "filename":   safe_name,
        "size_bytes": size,
        "size_mb":    size_mb,
        "sha256":     sha256,
...
def admin_update():
    ...
    if request.method == "POST":
        ...
        if action == "delete":
            ...
            # Mettre à jour uniquement le changelog sans re-uploader
            meta = _get_update_meta()
            if meta.get("version"):
...
def _page_update(meta, history, msg, admin_user, admin_role):
    ...
    sha_short   = (meta.get("sha256") or "")[:16]

    # Bloc update active
    if has_update:
        cl_text = meta.get("changelog","") or ""
        current_block = f"""
<div class="card card-glow" style="border-color:rgba(0,255,136,.3)">
  ...
  <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(140px,1fr));gap:10px;margin-bottom:14px">
    <div class="card-sm"><div style="font-size:10px;color:var(--muted)">FICHIER</div><div style="font-size:12px;word-break:break-all;color:var(--text)">{meta['filename']}</div></div>
    <div class="card-sm"><div style="font-size:10px;color:var(--muted)">TAILLE</div><div style="font-size:13px;font-weight:700;color:var(--text)">{size_str}</div></div>
    <div class="card-sm"><div style="font-size:10px;color:var(--muted)">UPLOADÉ</div><div style="font-size:12px;color:var(--text)">{uploaded_at}</div></div>
    <div class="card-sm"><div style="font-size:10px;color:var(--muted)">PAR</div><div style="font-size:12px;color:var(--text)">{meta.get('uploaded_by','—')}</div></div>
    {'<div class="card-sm"><div style="font-size:10px;color:var(--muted)">SHA256</div><div style="font-family:monospace;font-size:10px;color:var(--muted)">'+sha_short+'…</div></div>' if sha_short else ''}
  </div>
  {"<div style='background:var(--card2);border-radius:8px;padding:10px;margin-bottom:12px'><div style='font-size:10px;color:var(--muted);margin-bottom:6px'>CHANGELOG</div><pre style='font-size:12px;color:var(--text2);white-space:pre-wrap;margin:0;font-family:inherit;line-height:1.7'>"+cl_text+"</pre></div>" if cl_text else ''}
  ...
  <div id="cl-edit" style="display:none;margin-top:12px">
    <form method="POST">
      ...
      <textarea name="changelog" rows="5" style="width:100%;background:var(--bg);border:1px solid var(--border);border-radius:8px;padding:10px;color:var(--text);font-size:13px;resize:vertical;outline:none;font-family:inherit">{cl_text}</textarea>
...
<div class="card" style="border-color:rgba(255,68,102,.15)">
  <div style="text-align:center;padding:28px;color:var(--muted)">
    ...
    hist_rows = ""
    for h in history[:15]:
        sz  = f"{h.get('size_bytes',0)/1024/1024:.1f} Mo" if h.get("size_bytes") else "—"
        dt  = fmt_ts(h.get("uploaded_at")) if h.get("uploaded_at") else "—"
        cl2 = (h.get("changelog","") or "").split("\n")[0][:60]
        hist_rows += f'<tr><td style="font-weight:700;color:var(--green)">v{h.get("version","?")}</td><td style="font-size:11px">{dt}</td><td style="font-size:11px">{sz}</td><td style="font-size:11px;color:var(--text2);max-width:240px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">{cl2}</td><td style="font-size:11px;color:var(--muted)">{h.get("uploaded_by","—")}</td></tr>'
...
<div>
  <div class="card">
    ...
    <p style="font-size:12px;color:var(--text2);margin-bottom:14px;line-height:1.7">
      Sélectionne ton <b style="color:var(--text)">.exe</b> (ou .py / .zip) depuis ton PC.<br>
      Le fichier est hébergé <b style="color:var(--green)">directement sur ce serveur</b> — pas besoin de GitHub.
    </p>
    ...
    <div id="drop-zone" style="border:2px dashed var(--border);border-radius:12px;padding:32px;text-align:center;cursor:pointer;transition:all .2s;margin-bottom:14px" onclick="document.getElementById('file-input').click()">
      ...
      <div id="drop-label" style="font-size:14px;color:var(--text2);margin-top:10px">Clique ou glisse ton fichier .exe ici</div>
    ...
    <div id="upload-progress-wrap" style="display:none;margin-top:12px">
      ...
      <div style="background:var(--card2);border-radius:20px;height:8px;overflow:hidden">
        <div id="upload-bar" style="height:100%;background:linear-gradient(90deg,var(--green),var(--cyan));border-radius:20px;width:0%;transition:width .2s"></div>
      </div>
  ...
  <div class="card" style="margin-top:14px">
    ...
    <div style="font-size:12px;color:var(--text2);line-height:2.2">
      ...
      <div style="display:flex;justify-content:space-between;padding:6px 0">
        ...
        <span>Télécharger le .exe</span>
      </div>
    </div>
    <div style="margin-top:10px;background:var(--card2);border-radius:8px;padding:10px;font-size:11px">
      <b style="color:var(--text)">Payload check :</b>&nbsp;
      <code style="color:var(--green)">{{"version": "7.0"}}</code>
    </div>
    <div style="margin-top:8px;font-size:11px;color:var(--muted)">
      ✅ Supporte les requêtes <b>Range</b> (reprise de téléchargement).<br>
      ✅ SHA256 vérifié à l'upload et loggé.
    </div>
  </div>
</div>

<!-- COLONNE DROITE : Etat actuel + historique -->
<div>
  {current_block}
  {hist_block}
</div>
...
(function() {{
  ...
  dropZone.addEventListener('dragover', function(e) {{
    ...
    dropZone.style.background  = 'rgba(0,255,136,.05)';
  }});
  ...
  window.doUpload = function() {{
    ...
    xhr.upload.addEventListener('progress', function(e) {{
      ...
      var rem = e.total - e.loaded;
      var eta = elapsed > 0 && e.loaded > 0 ? Math.round(rem / (e.loaded / elapsed)) : '?';
    ...
    xhr.onload = function() {{
      btn.disabled    = false;
      btn.textContent = '⬆️ Publier la mise à jour';
      try {{
        ...
        if (resp.success) {{
          ...
          status.textContent = '❌ ' + (resp.msg || 'Erreur inconnue');
