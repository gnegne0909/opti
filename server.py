...
APP_VERSION     = "4.0"
MIN_VERSION     = os.environ.get("MIN_VERSION", "8.0")  # Bloquer les versions client inferieures a cette valeur
...
def init_db():
    ...
    for k, v in [("maintenance","0"),("vpn_block","0"),("min_version", MIN_VERSION),
...
@app.route("/api/min_version")
def api_min_version():
    """Retourne la version minimale autorisee. Le client se bloque s'il est trop vieux."""
    min_ver = get_setting("min_version", MIN_VERSION)
    return jsonify({"min_version": min_ver})
...
def api_update_check():
    ...
    return jsonify({
        ...
        "download_url": "/api/update/download" if has_update else None,
        "min_version": min_ver,
    })
...
def admin_update():
    ...
    if request.method == "POST":
        ...
        if action == "set_min_version":
            import re as _re
            new_min = request.form.get("min_version", "").strip()
            if _re.match(r'^\d+\.\d+$', new_min):
                set_setting("min_version", new_min)
                add_log("OK","UPDATE",f"min_version -> {new_min} par {session['admin_user']}",session["admin_user"])
                msg = f"Version minimale mise a jour -> v{new_min}"
            else:
                msg = "Format invalide (ex: 8.0)"
        elif action == "upload":
...
<div>
  {current_block}
  <div class="card">
    <div class="card-title">&#128274; Version minimale client</div>
    <p style="font-size:12px;color:var(--text2);margin-bottom:12px;line-height:1.6">
      Les clients dont la version est <b>inferieure</b> a cette valeur seront
      <b style="color:var(--red)">bloques au demarrage</b>.
    </p>
    <form method="POST" style="display:flex;gap:8px;align-items:center">
      <input type="hidden" name="action" value="set_min_version">
      <input name="min_version" placeholder="8.0" value="{get_setting('min_version', MIN_VERSION)}"
             style="width:90px;background:var(--card2);border:1px solid var(--border);border-radius:8px;
             padding:8px 10px;color:var(--text);font-size:14px;font-weight:700;outline:none">
      <button type="submit" class="btn btn-sm btn-yellow">Sauvegarder</button>
    </form>
    <div style="font-size:11px;color:var(--muted);margin-top:8px">Actuellement : <b style="color:var(--yellow)">v{get_setting('min_version', MIN_VERSION)}</b> &middot; <code style="color:var(--green)">GET /api/min_version</code></div>
  </div>
...
# ==============================================================================
#  ESPACE CLIENT WEB  /client
#  Auth partagee avec la table `users` -- meme username/password que le .exe
# ==============================================================================

_CLIENT_LOGIN_HTML = """<!DOCTYPE html>
<html lang="fr"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>Espace Client - WinOptimizer</title>
<style>
:root{--bg:#07070d;--card:#0f0f1a;--card2:#161625;--border:#252538;--green:#00ff88;--cyan:#06b6d4;
      --blue:#3b82f6;--text:#e8e8f4;--text2:#9898b8;--muted:#55556a;--red:#ff4466;}
*{box-sizing:border-box;margin:0;padding:0}
body{font-family:'Inter',system-ui,sans-serif;background:var(--bg);color:var(--text);
     display:flex;align-items:center;justify-content:center;min-height:100vh;
     background:radial-gradient(ellipse at 70% 80%,#080e20,var(--bg) 60%)}
.box{width:100%;max-width:400px;padding:16px}
.card{background:var(--card);border:1px solid var(--border);border-radius:18px;padding:36px 30px}
.logo{text-align:center;margin-bottom:28px}
.logo-icon{width:58px;height:58px;background:linear-gradient(135deg,#3b82f6,#06b6d4);border-radius:16px;
           display:inline-flex;align-items:center;justify-content:center;font-size:28px;color:#fff;
           margin-bottom:12px;box-shadow:0 0 30px rgba(59,130,246,.25)}
.logo h2{font-size:22px;font-weight:800}.logo p{color:var(--muted);font-size:13px;margin-top:4px}
.alert-err{padding:11px;border-radius:8px;background:rgba(255,68,102,.1);
           border:1px solid rgba(255,68,102,.3);color:var(--red);font-size:13px;margin-bottom:14px}
.form-group{margin-bottom:12px}
label{display:block;font-size:12px;font-weight:600;color:var(--text2);margin-bottom:5px}
input{width:100%;background:var(--card2);border:1px solid var(--border);border-radius:8px;
...
.btn{width:100%;padding:12px;border-radius:9px;background:linear-gradient(135deg,var(--blue),var(--cyan));
     color:#fff;font-size:14px;font-weight:700;border:none;cursor:pointer;margin-top:6px;
...
<div class="box"><div class="card">
  <div class="logo">
    <div class="logo-icon">&#128100;</div>
    <h2>Espace Client</h2>
    <p>WinOptimizer Pro</p>
  </div>
  {% if error %}<div class="alert-err">{{ error }}</div>{% endif %}
  <form method="POST">
    <div class="form-group"><label>Nom d'utilisateur</label>
      <input name="username" placeholder="votre_username" autofocus autocomplete="username"></div>
    <div class="form-group"><label>Mot de passe</label>
      <input name="password" type="password" placeholder="&bull;&bull;&bull;&bull;&bull;&bull;&bull;&bull;" autocomplete="current-password"></div>
    <button type="submit" class="btn">Se connecter &rarr;</button>
  </form>
  <p class="hint">Utilisez les memes identifiants que votre application</p>
</div></div></body></html>"""


_CLIENT_DASH_HTML = """<!DOCTYPE html>
<html lang="fr"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>Mon Espace - WinOptimizer</title>
<style>
:root{--bg:#07070d;--card:#0f0f1a;--card2:#161625;--border:#252538;--green:#00ff88;--cyan:#06b6d4;
      --blue:#3b82f6;--text:#e8e8f4;--text2:#9898b8;--muted:#55556a;--red:#ff4466;--yellow:#fbbf24;}
*{box-sizing:border-box;margin:0;padding:0}
body{font-family:'Inter',system-ui,sans-serif;background:var(--bg);color:var(--text);min-height:100vh}
nav{background:var(--card);border-bottom:1px solid var(--border);padding:0 24px;
    display:flex;align-items:center;justify-content:space-between;height:56px;position:sticky;top:0;z-index:10}
.nav-brand{display:flex;align-items:center;gap:10px;font-weight:800;font-size:16px}
.nav-icon{width:32px;height:32px;background:linear-gradient(135deg,var(--blue),var(--cyan));
...
.b-green{background:rgba(0,255,136,.1);color:var(--green);border:1px solid rgba(0,255,136,.2)}
.b-gray{background:rgba(150,150,180,.1);color:var(--text2);border:1px solid var(--border)}
.b-red{background:rgba(255,68,102,.1);color:var(--red);border:1px solid rgba(255,68,102,.3)}
.logout{color:var(--text2);font-size:13px;text-decoration:none;padding:6px 12px;
        border-radius:7px;border:1px solid var(--border);transition:.15s}
.logout:hover{background:var(--card2);color:var(--red)}
...
.stat-big{font-size:32px;font-weight:800;margin-bottom:4px}
.stat-lbl{font-size:12px;color:var(--muted)}
.info-row{display:flex;justify-content:space-between;align-items:center;
          padding:8px 0;border-bottom:1px solid var(--border);font-size:13px}
.info-row:last-child{border-bottom:none}
.info-key{color:var(--text2)}
.info-val{font-weight:600;font-family:monospace;font-size:12px}
.addon-chip{display:inline-flex;align-items:center;gap:6px;padding:5px 10px;
            background:rgba(0,255,136,.05);border:1px solid rgba(0,255,136,.2);
            border-radius:8px;font-size:12px;font-weight:600;margin:3px;color:var(--green)}
.hw-item{padding:8px 10px;background:var(--card2);border-radius:8px;font-size:11px;
         color:var(--text2);margin-bottom:6px}
.hw-item b{color:var(--text);display:block;margin-bottom:2px;font-size:12px}
table{width:100%;border-collapse:collapse}
th{font-size:11px;color:var(--muted);font-weight:600;text-align:left;
   padding:6px 8px;border-bottom:1px solid var(--border)}
td{font-size:12px;padding:7px 8px;border-bottom:1px solid rgba(37,37,56,.6);color:var(--text2)}
td b{color:var(--text)}
...
<nav>
  <div class="nav-brand">
    ...
    WinOptimizer
    <span class="badge b-gray" style="font-size:10px">Espace Client</span>
  </div>
  <div style="display:flex;align-items:center;gap:12px">
    <span style="font-size:13px;color:var(--text2)">&#128100; <b style="color:var(--text)">{{ username }}</b></span>
    <span class="badge {{ 'b-blue' if plan == 'PRO' else 'b-gray' }}">{{ plan }}</span>
    <a href="/client/logout" class="logout">Deconnexion</a>
  </div>
</nav>
<main>
  <h1>Bonjour, {{ username }} &#128075;</h1>
  <p class="subtitle">Bienvenue dans votre espace personnel WinOptimizer</p>

  <div class="grid-3">
    <div class="card">
      <div class="stat-big" style="color:{{ '#3b82f6' if plan=='PRO' else 'var(--cyan)' }}">{{ plan }}</div>
      <div class="stat-lbl">Plan actuel</div>
    </div>
    <div class="card">
      <div class="stat-big" style="color:var(--green)">{{ connections }}</div>
      <div class="stat-lbl">Connexions totales</div>
    </div>
    <div class="card">
      <div class="stat-big" style="color:var(--yellow)">{{ addons|length }}</div>
      <div class="stat-lbl">Addons actifs</div>
    </div>
  </div>
  ...
  <div class="grid-2">
    <div class="card">
      ...
      <div class="info-row"><span class="info-key">Plan</span>
        <span class="badge {{ 'b-blue' if plan=='PRO' else 'b-gray' }}">{{ plan }}</span></div>
      <div class="info-row"><span class="info-key">Cle de licence</span>
        <span class="info-val" style="font-size:10px">{{ license_key or 'N/A' }}</span></div>
      <div class="info-row"><span class="info-key">Discord</span>
        <span class="info-val">{{ discord_id or 'Non lie' }}</span></div>
      <div class="info-row"><span class="info-key">Inscrit le</span>
        <span class="info-val">{{ created_at }}</span></div>
      <div class="info-row"><span class="info-key">Derniere connexion</span>
        <span class="info-val">{{ last_login }}</span></div>
    </div>
    <div class="card">
      <div class="card-title">&#128187; Configuration detectee</div>
      {% if cpu_info %}<div class="hw-item"><b>CPU</b>{{ cpu_info }}</div>{% endif %}
      {% if gpu_info %}<div class="hw-item"><b>GPU</b>{{ gpu_info }}</div>{% endif %}
      {% if ram_info %}<div class="hw-item"><b>RAM</b>{{ ram_info }}</div>{% endif %}
      {% if os_info  %}<div class="hw-item"><b>OS</b>{{ os_info }}</div>{% endif %}
      {% if disk_info%}<div class="hw-item"><b>Stockage</b>{{ disk_info }}</div>{% endif %}
      {% if not cpu_info and not gpu_info %}
        <div style="color:var(--muted);font-size:12px;text-align:center;padding:20px">
          Lancez l'application pour detecter votre config</div>
      {% endif %}
    </div>
  </div>

  <div class="card" style="margin-bottom:16px">
    <div class="card-title">&#129513; Addons actifs ({{ addons|length }})</div>
    {% if addons %}
      {% for a in addons %}<span class="addon-chip">&#10003; {{ a }}</span>{% endfor %}
    {% else %}
      <div style="color:var(--muted);font-size:12px">Aucun addon active pour le moment.</div>
    {% endif %}
  </div>

  <div class="card">
    <div class="card-title">&#128336; Historique des connexions (30 dernieres)</div>
    {% if login_history %}
    <div style="overflow-x:auto">
    <table>
      <thead><tr><th>Date</th><th>IP</th><th>Localisation</th><th>Statut</th></tr></thead>
      <tbody>
        {% for e in login_history %}
        <tr>
          <td><b>{{ e.date }}</b></td>
          <td style="font-family:monospace">{{ e.ip }}</td>
          <td>{{ e.flag }} {{ e.city }}, {{ e.country }}</td>
          <td>{% if e.success %}<span class="badge b-green">OK</span>
              {% else %}<span class="badge b-red">Echec</span>{% endif %}</td>
        </tr>
        {% endfor %}
      </tbody>
    </table>
    ...
    {% else %}
    <div style="color:var(--muted);font-size:12px;text-align:center;padding:16px">Aucun historique</div>
    {% endif %}
  </div>
</main></body></html>"""


def _fmt_ts_client(ts):
...
def _client_required(f):
    ...
    @wraps(f)
    def decorated(*args, **kwargs):
        if not session.get("client_logged"):
...
def client_login():
    ...
    if request.method == "POST":
        ...
        if not username or not password:
            ...
            if ip_rule and ip_rule["rule"] == "blacklist":
                conn.close()
                error = "Acces refuse depuis cette IP"
            else:
                row = conn.execute("SELECT * FROM users WHERE username=?", (username,)).fetchone()
                if not row:
                    conn.close()
                    ...
                    conn.close()
                    error = "Compte suspendu temporairement."
                else:
                    ph = _hash_password(password)
                    valid = (ph == row["password_hash"]) or (
                        row["must_change_pass"] and row["temp_password"]
                        and password == row["temp_password"]
                    )
                    conn.close()
                    if not valid:
                        error = "Identifiants incorrects"
                        ...
                        add_log("OK", "CLIENT_WEB", f"Connexion espace client: {username} IP={ip}", username)
                        return redirect("/client")
    return render_template_string(_CLIENT_LOGIN_HTML, error=error)
...
def client_dashboard():
    ...
    row = conn.execute("SELECT * FROM users WHERE username=?", (username,)).fetchone()
    if not row:
        conn.close(); session.clear(); return redirect("/client/login")

    addon_rows = conn.execute(
        "SELECT addon_key FROM user_addons WHERE username=?", (username,)).fetchall()
    active_addons = [r["addon_key"] for r in addon_rows]
    plan = row["plan"] if row["plan"] in ("NORMAL","PRO") else "PRO"
    if plan == "PRO":
        for a in PRO_FREE_ADDONS:
            if a not in active_addons: active_addons.append(a)

    history_rows = conn.execute(
        "SELECT ip,ts,country,city,flag,success,reason FROM login_history "
        "WHERE username=? ORDER BY ts DESC LIMIT 30", (username,)).fetchall()
    conn.close()

    login_history = [
        {"date": _fmt_ts_client(r["ts"]), "ip": r["ip"] or "?",
         "country": r["country"] or "?", "city": r["city"] or "?",
         "flag": r["flag"] or "?", "success": bool(r["success"]),
         "reason": r["reason"] or ""}
        for r in history_rows
    ]

    return render_template_string(_CLIENT_DASH_HTML,
        username=username, plan=plan,
        connections=row["connections"] or 0,
        license_key=row["license_key"] or "",
        discord_id=row["discord_id"] or "",
        created_at=_fmt_ts_client(row["created_at"]),
        last_login=_fmt_ts_client(row["last_login"]),
        cpu_info=row["cpu_info"] or "",  gpu_info=row["gpu_info"] or "",
        ram_info=row["ram_info"] or "",  os_info=row["os_info"] or "",
        disk_info=row["disk_info"] or "",
        addons=active_addons, login_history=login_history,
    )


@app.route("/client/logout")
def client_logout():
    session.pop("client_logged", None)
