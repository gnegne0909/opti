...
def api_support_frame():
    ...
    with _remote_lock:
        ...
        # Retourner les commandes en attente + statut session
        cmds = list(sess.get("commands", []))
        sess["commands"] = []
        sess_status = sess.get("status", "pending")
    return jsonify({"success": True, "commands": cmds, "session_status": sess_status})
...
def api_addon_activate():
    """
    Le client envoie:
      - addon_license_key : la clé générée par generate_addon_key (ex: ABCD-EFGH-...)
      - addon_key         : le nom de l'addon (ex: "crosshair", "fps_counter")
      - username          : le nom d'utilisateur
    """
    data = request.get_json(silent=True) or {}
    username         = data.get("username","").strip().lower()
    addon_name       = data.get("addon_key","").strip()          # nom addon ex: "crosshair"
    addon_license_key = data.get("addon_license_key","").strip().upper()  # la clé addon générée

    if not addon_license_key or not addon_name:
        return jsonify({"success": False, "reason": "Paramètres manquants (addon_license_key, addon_key requis)"})

    conn = get_db()
    # La table addon_licenses: key=clé addon générée, addon_key=nom de l'addon
    row = conn.execute(
    ...
    if not row:
        conn.close()
        return jsonify({"success": False, "reason": "Clé addon invalide ou ne correspond pas à cet addon"})
    if row["status"] == "used":
        conn.close()
        return jsonify({"success": False, "reason": "Clé addon déjà utilisée"})
    if row["status"] != "active":
        conn.close()
        return jsonify({"success": False, "reason": f"Clé addon {row['status']}"})
...
def admin_remote_command(session_id):
    ...
    with _remote_lock:
        sess = _remote_sessions.get(session_id)
        if sess and sess.get("status") in ("active", "pending"):
...
def internal_stats():
    ...
    d = {"total":conn.execute("SELECT COUNT(*) FROM users").fetchone()[0],
         "active":conn.execute("SELECT COUNT(*) FROM users WHERE status='active'").fetchone()[0],
...
function setupRemoteInput(sessionId){
  ...
  // Mouse move
  // Calcul coords en tenant compte de l'aspect ratio (object-fit:contain)
  function _getImgCoords(e, img, overlay){
    var cr = overlay.getBoundingClientRect();
    ...
    var mx = e.clientX - cr.left - ox;
    var my = e.clientY - cr.top  - oy;
    var px = Math.max(0, Math.min(100, Math.round(mx/rw*100)));
    var py = Math.max(0, Math.min(100, Math.round(my/rh*100)));
    return {x:px, y:py};
  }
  overlay.addEventListener('mousemove',function(e){
    var p=_getImgCoords(e,img,overlay);
    sendRemoteCmd(sessionId,'MOVE:'+p.x+':'+p.y);
  });
  overlay.addEventListener('click',function(e){
    var p=_getImgCoords(e,img,overlay);
    sendRemoteCmd(sessionId,'CLICK:'+p.x+':'+p.y);
  });
  overlay.addEventListener('contextmenu',function(e){
    e.preventDefault();
    var p=_getImgCoords(e,img,overlay);
    sendRemoteCmd(sessionId,'RCLICK:'+p.x+':'+p.y);
  });
...
def _page_users(users, q, plan_f, status_f, admin_user, admin_role):
    rows = ""
    for u in users:
        pb = '<span class="badge b-blue">PRO</span>' if u["plan"]=="PRO" else '<span class="badge b-gray">NORMAL</span>'
        if u["status"]=="active": sb='<span class="badge b-green">actif</span>'
        elif u["status"]=="banned": sb='<span class="badge b-red">banni</span>'
        else: sb=f'<span class="badge b-yellow">{u["status"]}</span>'
        geo = get_geoip(u["ip"]) if u["ip"] else {}
        flag = geo.get("flag","")
        tags_html = " ".join(f'<span class="tag">{t.strip()}</span>' for t in (u["tags"] or "").split(",") if t.strip()) if u.get("tags") else ""
        rows += f"""<tr>
...
def _page_user_detail(user, logs, alerts, login_hist, notes, remote_hist, geo, msg, admin_user, admin_role):
    ...
    # Tags
    tags = [t.strip() for t in (user.get("tags") or "").split(",") if t.strip()]
...
def _page_keys(keys, addon_keys, admin_user, admin_role):
    ...
    for k in keys:
        ...
        pb = '<span class="badge b-blue">PRO</span>' if k["plan"]=="PRO" else '<span class="badge b-gray">NORMAL</span>'
    ...
    addon_rows = ""
    for a in addon_keys:
        sc = '<span class="badge b-green">active</span>' if a["status"]=="active" else f'<span class="badge b-gray">{a["status"]}</span>'
        addon_rows += f'<tr><td style="font-family:monospace;font-size:11px">{a["key"]}</td><td><span class="badge b-purple">{a["addon_key"]}</span></td><td>{sc}</td><td style="font-size:11px;color:var(--text2)">{fmt_ts(a["created_at"])}</td><td style="font-size:11px;color:var(--text2)">{a["note"] or "—"}</td></tr>'

    addon_options = "\n".join(f'<option value="{k}">{k}</option>' for k in ["crosshair","fps_counter","vibrance","ram_cleaner","overclock","antilag"])
...
<div class="card">
  ...
  <div class="tbl-wrap"><table>
    <thead><tr><th>Clé</th><th>Addon</th><th>Statut</th><th>Créée</th><th>Note</th></tr></thead>
    <tbody>{addon_rows or '<tr><td colspan="5" style="text-align:center;padding:20px;color:var(--muted)">Aucune clé addon</td></tr>'}</tbody>
  </table></div>
</div>"""
