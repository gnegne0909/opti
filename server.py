...
def admin_user_action(username):
    """Route POST classique (redirect) — conservée pour compatibilité."""
    action = request.form.get("action")
    result = _do_user_action(username, action, session["admin_user"])
    if result.get("redirect"):
        return redirect(result["redirect"])
    return redirect(f"/admin/user/{username}?msg={result.get('msg','OK').replace(' ', '+')}")

@app.route("/admin/user/<username>/action_ajax", methods=["POST"])
@admin_required
def admin_user_action_ajax(username):
    """Route AJAX — retourne JSON, utilisée par les boutons du panel."""
    data   = request.get_json(silent=True) or {}
    action = data.get("action") or request.form.get("action")
    result = _do_user_action(username, action, session["admin_user"])
    return jsonify(result)

def _do_user_action(username: str, action: str, admin_user: str) -> dict:
    """Logique centrale des actions admin sur un user. Retourne un dict résultat."""
    conn = get_db()
    user = conn.execute("SELECT * FROM users WHERE username=?", (username,)).fetchone()
    if not user:
        conn.close()
    ...
    try:
        if action == "suspend":
            ...
            conn.commit()
            add_log("WARN","ADMIN",f"Suspendu: {username}", admin_user)
            _push_sse({"type":"user_action","action":"suspend","username":username,"admin":admin_user})
            conn.close()
            return {"success": True, "msg": f"✅ {username} suspendu", "new_status": "suspended"}

        elif action == "ban":
            conn.execute("UPDATE users SET status='banned' WHERE username=?", (username,))
            conn.commit()
            add_log("WARN","ADMIN",f"Banni: {username}", admin_user)
            _push_sse({"type":"user_action","action":"ban","username":username,"admin":admin_user})
            conn.close()
            return {"success": True, "msg": f"🚫 {username} banni", "new_status": "banned"}

        elif action == "reactivate":
            conn.execute("UPDATE users SET status='active' WHERE username=?", (username,))
            conn.commit()
            add_log("OK","ADMIN",f"Réactivé: {username}", admin_user)
            _push_sse({"type":"user_action","action":"reactivate","username":username,"admin":admin_user})
            conn.close()
            return {"success": True, "msg": f"✅ {username} réactivé", "new_status": "active"}

        elif action == "reset_hwid":
            old_hwid = user["hwid"] or "—"
            conn.execute("UPDATE users SET hwid=NULL WHERE username=?", (username,))
            conn.commit()
            add_log("OK","ADMIN",f"HWID reset: {username} (était: {old_hwid[:16]}…)", admin_user)
            _push_sse({"type":"user_action","action":"reset_hwid","username":username,"admin":admin_user})
            conn.close()
            return {"success": True, "msg": f"🔄 HWID réinitialisé pour {username}"}
            ...
            if discord_id:
                _notify_discord(discord_id, f"🔑 MDP temporaire: `{temp}` — Change-le à la connexion!")
            add_log("OK","ADMIN",f"MDP reset: {username} temp={temp}", admin_user)
            conn.close()
            return {"success": True, "msg": f"🔑 MDP temporaire : {temp}", "temp_pass": temp,
                    "redirect": f"/admin/user/{username}?msg=MDP+temp:+{temp}"}

        elif action in ("upgrade_pro", "upgrade_lifetime"):
            conn.execute("UPDATE users SET plan='PRO' WHERE username=?", (username,))
            conn.commit()
            add_log("OK","ADMIN",f"Upgrade PRO: {username}", admin_user)
            _push_sse({"type":"user_action","action":"upgrade_pro","username":username,"admin":admin_user})
            conn.close()
            return {"success": True, "msg": f"⭐ {username} passé en PRO", "new_plan": "PRO"}

        elif action == "downgrade_normal":
            conn.execute("UPDATE users SET plan='NORMAL' WHERE username=?", (username,))
            conn.commit()
            add_log("OK","ADMIN",f"Downgrade NORMAL: {username}", admin_user)
            _push_sse({"type":"user_action","action":"downgrade","username":username,"admin":admin_user})
            conn.close()
            return {"success": True, "msg": f"⬇ {username} passé en NORMAL", "new_plan": "NORMAL"}

        elif action == "ban_ip":
            ip = user["ip"]
            if not ip:
                conn.close()
                return {"success": False, "msg": "IP inconnue pour cet utilisateur"}
            conn.execute("INSERT OR REPLACE INTO ip_rules (ip,rule,note,added_at) VALUES (?,?,?,?)",
                         (ip, "blacklist", f"Banni depuis profil {username}", int(time.time())))
            conn.commit()
            add_log("WARN","ADMIN",f"IP {ip} bannie (via {username})", admin_user)
            conn.close()
            return {"success": True, "msg": f"🚫 IP {ip} bannie"}
            ...
            if not ip:
                ...
                return {"success": False, "msg": "IP inconnue pour cet utilisateur"}
            conn.execute("INSERT OR REPLACE INTO ip_rules (ip,rule,note,added_at) VALUES (?,?,?,?)",
                         (ip, "whitelist", f"Whitelisté depuis profil {username}", int(time.time())))
            conn.commit()
            add_log("OK","ADMIN",f"IP {ip} whitelistée (via {username})", admin_user)
            conn.close()
            return {"success": True, "msg": f"✅ IP {ip} whitelistée"}

        else:
            conn.close()
            return {"success": False, "msg": f"Action inconnue : {action}"}

    except Exception as e:
        try: conn.close()
        except: pass
        add_log("ERROR","ADMIN",f"Action {action} sur {username} échouée: {e}", admin_user)
        return {"success": False, "msg": f"Erreur : {str(e)[:80]}"}
...
<div class="card">
  ...
  <div id="action-msg-{user['username']}" style="min-height:28px;font-size:13px;font-weight:600;margin-bottom:8px"></div>
  <div style="display:flex;flex-wrap:wrap;gap:6px">
    <button onclick="userAction(this,'reactivate')" class="btn btn-green btn-sm">✅ Activer</button>
    <button onclick="userAction(this,'suspend')" class="btn btn-ghost btn-sm">⏸ Suspendre</button>
    <button onclick="userAction(this,'ban')" class="btn btn-red btn-sm">🚫 Bannir</button>
    <button onclick="userAction(this,'reset_hwid')" class="btn btn-ghost btn-sm">🔄 Reset HWID</button>
    <button onclick="userAction(this,'reset_password')" class="btn btn-ghost btn-sm">🔑 Reset MDP</button>
    <button onclick="userAction(this,'upgrade_pro')" class="btn btn-sm btn-blue">⭐ PRO</button>
    <button onclick="userAction(this,'downgrade_normal')" class="btn btn-sm btn-gray">⬇ NORMAL</button>
    <button onclick="userAction(this,'ban_ip')" class="btn btn-red btn-sm">🚫 Ban IP</button>
    <button onclick="userAction(this,'whitelist_ip')" class="btn btn-green btn-sm">✅ Whitelist IP</button>
  </div>
<script>
(function() {{
  var UN = {json.dumps(user['username'])};
  var UI = {json.dumps(user['ip'] or '')};
  var CONFIRM_MAP = {{
    ban: 'Bannir ' + UN + ' ?',
    suspend: 'Suspendre ' + UN + ' ?',
    reset_hwid: 'Réinitialiser le HWID de ' + UN + ' ?',
    reset_password: 'Générer un MDP temporaire pour ' + UN + ' ?',
    upgrade_pro: 'Passer ' + UN + ' en PRO ?',
    downgrade_normal: 'Repasser ' + UN + ' en NORMAL ?',
    ban_ip: 'Bannir l\\'IP ' + (UI||'inconnue') + ' ?',
    whitelist_ip: 'Whitelister l\\'IP ' + (UI||'inconnue') + ' ?',
  }};
  window.userAction = function(btn, action) {{
    var cm = CONFIRM_MAP[action];
    if (cm && !confirm(cm)) return;
    var orig = btn.textContent;
    btn.disabled = true;
    btn.textContent = '⟳ …';
    var msgEl = document.getElementById('action-msg-' + UN);
    fetch('/admin/user/' + UN + '/action_ajax', {{
      method: 'POST',
    ...
    .then(function(data) {{
      ...
      if (msgEl) {{
        if (data.temp_pass) {{
          msgEl.innerHTML = '🔑 MDP temporaire\u00a0: <code style="background:var(--card2);padding:2px 8px;border-radius:4px;font-size:13px;user-select:all">' + data.temp_pass + '</code>&nbsp;<button onclick="navigator.clipboard.writeText(\\'' + data.temp_pass + '\\').then(function(){{this.textContent=\\'✅ Copié\\'}}.bind(this))" style="background:none;border:1px solid var(--border);color:var(--text2);padding:2px 8px;border-radius:4px;cursor:pointer;font-size:11px">📋 Copier</button>';
          msgEl.style.color = 'var(--gold)';
        }} else {{
          msgEl.textContent = data.msg || (data.success ? '✅ OK' : '❌ Erreur');
          msgEl.style.color = data.success ? 'var(--green)' : 'var(--red)';
          setTimeout(function() {{ if(msgEl) msgEl.textContent=''; }}, 5000);
        }}
      }}
      if (data.success) {{
        if (data.new_plan) {{
          var pb = document.querySelector('[data-plan-badge]');
          if (pb) pb.textContent = data.new_plan;
        }}
        if (data.new_status) {{
          var sb = document.querySelector('[data-status-badge]');
          if (sb) {{
            sb.textContent = data.new_status;
            sb.style.color = data.new_status==='active' ? 'var(--green)' : data.new_status==='banned' ? 'var(--red)' : 'var(--gold)';
          }}
        }}
      }}
    }})
      ...
      .then(r=>r.json()).then(d=>{{
        ...
        if(d.chat && d.chat.length){{
          d.chat.forEach(function(m){{
            if(m.ts>_lastChatTs){{ addChat(m.text,'user'); _lastChatTs=m.ts; }}
          }});
        }}
      }}).catch(()=>{{}});
  }}, 80);
...
UPDATE_META_BETA = os.path.join(UPDATE_DIR, "meta_beta.json")
UPDATE_HISTORY   = os.path.join(UPDATE_DIR, "history.json")

def _ensure_update_dir():
    os.makedirs(UPDATE_DIR, exist_ok=True)

def _get_update_meta(beta=False):
    path = UPDATE_META_BETA if beta else UPDATE_META_FILE
    try:
        with open(path, "r") as f:
            return json.load(f)
    except Exception:
        return {"version": None, "filename": None, "github_url": None,
                "uploaded_at": None, "uploaded_by": None, "changelog": "",
                "size_bytes": 0, "is_beta": beta}

def _save_update_meta(meta, beta=False):
    _ensure_update_dir()
    path = UPDATE_META_BETA if beta else UPDATE_META_FILE
    with open(path, "w") as f:
        json.dump(meta, f, indent=2)

def _append_update_history(meta):
    """Sauvegarde chaque version publiée dans l'historique."""
    _ensure_update_dir()
    try:
        try:
            ...
            history = []
        # Éviter les doublons sur même version
        history = [h for h in history if h.get("version") != meta.get("version")]
        history.insert(0, meta)
        history = history[:50]  # Garder les 50 dernières versions
        with open(UPDATE_HISTORY, "w") as f:
            json.dump(history, f, indent=2)
    except Exception as e:
        print(f"[update_history] {e}")

def _get_update_history():
    try:
        with open(UPDATE_HISTORY, "r") as f:
            return json.load(f)
    except Exception:
...
def api_update_check():
    ...
    want_beta      = bool(data.get("beta", False))
    meta           = _get_update_meta()

    # Si bêta demandé, checker aussi le canal beta
    if want_beta:
        beta_meta = _get_update_meta(beta=True)
        # Utiliser le canal beta si version plus récente
        if beta_meta.get("version") and beta_meta.get("filename"):
            try:
                bv = [int(x) for x in str(beta_meta["version"]).split(".")]
                mv = [int(x) for x in str(meta.get("version","0") or "0").split(".")]
                if bv > mv:
                    meta = beta_meta
            except Exception:
                pass
    ...
    # Construire changelog structuré pour l'onglet MAJ du client
    raw_changelog = meta.get("changelog", "")
    changelog_entries = []
    if raw_changelog:
        # Format : lignes commençant par "- " ou "• " ou "✓ "
        lines = [l.strip() for l in raw_changelog.splitlines() if l.strip()]
        changelog_entries = [
            (f"v{server_version}", "Nouveautés de cette version", lines[:20])
        ]
    ...
    return jsonify({
        ...
        "changelog_entries": changelog_entries if has_update else [],
        "download_url":      meta.get("github_url") if has_update else None,
        "size_mb":           size_mb,
...
def api_update_download():
    """Redirige le client vers le fichier .exe hébergé sur GitHub Releases."""
    meta = _get_update_meta()
...
def admin_update():
    ...
    if request.method == "POST":
        ...
        is_beta  = request.form.get("is_beta", "0") == "1"

        if action == "upload":
            version    = request.form.get("version", "").strip()
            changelog  = request.form.get("changelog", "").strip()
            github_url = request.form.get("github_url", "").strip()
            size_mb_s  = request.form.get("size_mb", "0").strip()

            if not version:
                msg = "❌ Version requise (ex: 7.1)"
            elif not github_url or not github_url.startswith("https://"):
                msg = "❌ URL invalide. Doit commencer par https://"
            else:
                try:
                    size_bytes = int(float(size_mb_s) * 1024 * 1024) if size_mb_s else 0
                except Exception:
                    size_bytes = 0
                filename = github_url.split("/")[-1].split("?")[0] or "WinOptimizer.exe"
                meta = {
                    "version":     version,
                    "filename":    filename,
                ...
                _save_update_meta(meta, beta=is_beta)
                _append_update_history(meta)
                add_log("OK", "UPDATE",
                        f"Update v{version}{'(bêta)' if is_beta else ''} publiée → {github_url}",
                        session["admin_user"])
                _push_sse({"type": "update_pushed", "version": version,
                           "beta": is_beta, "admin": session["admin_user"]})
                canal = "BÊTA" if is_beta else "STABLE"
                msg = f"✅ Update v{version} [{canal}] publiée ! Les clients seront notifiés au prochain lancement."

        elif action == "delete":
            is_beta_del = request.form.get("is_beta_del","0") == "1"
            _save_update_meta({"version": None, "filename": None, "github_url": None,
                               "uploaded_at": None, "uploaded_by": None, "changelog": "",
                               "size_bytes": 0, "is_beta": is_beta_del}, beta=is_beta_del)
            canal = "bêta" if is_beta_del else "stable"
            add_log("WARN", "UPDATE", f"Update {canal} supprimée par {session['admin_user']}", session["admin_user"])
            msg = f"🗑️ Update {canal} supprimée."

    meta_stable = _get_update_meta(beta=False)
    meta_beta   = _get_update_meta(beta=True)
    history     = _get_update_history()
    return _page_update(meta_stable, meta_beta, history, msg, session["admin_user"], session["admin_role"])


def _page_update(meta_stable, meta_beta, history, msg, admin_user, admin_role):
    def _meta_block(meta, canal_label, canal_color, del_flag):
        has = bool(meta.get("filename") and meta.get("version"))
        size_bytes = meta.get("size_bytes") or 0
        size_str   = f"{size_bytes/1024/1024:.1f} Mo" if size_bytes else "—"
        up_at      = fmt_ts(meta.get("uploaded_at")) if has else "—"
        cl         = meta.get("changelog","") or "—"
        if not has:
            return f"""
<div class="card" style="border-color:rgba(255,68,102,.15)">
  <div class="card-title" style="color:{canal_color}">📭 {canal_label} — Aucune update publiée</div>
  <div style="color:var(--muted);font-size:13px;text-align:center;padding:16px">Les utilisateurs restent sur leur version actuelle</div>
</div>"""
        return f"""
<div class="card card-glow" style="border-color:rgba(0,255,136,.25)">
  <div class="card-title" style="color:{canal_color}">✅ {canal_label} — Update active</div>
  <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(150px,1fr));gap:10px;margin-bottom:14px">
    <div class="card-sm"><div style="font-size:11px;color:var(--muted)">VERSION</div><div style="font-size:22px;font-weight:800;color:{canal_color}">v{meta['version']}</div></div>
    <div class="card-sm"><div style="font-size:11px;color:var(--muted)">FICHIER</div><div style="font-size:12px;word-break:break-all">{meta['filename']}</div></div>
    <div class="card-sm"><div style="font-size:11px;color:var(--muted)">TAILLE</div><div style="font-size:13px">{size_str}</div></div>
    <div class="card-sm"><div style="font-size:11px;color:var(--muted)">PUBLIÉ LE</div><div style="font-size:12px">{up_at}</div></div>
    <div class="card-sm"><div style="font-size:11px;color:var(--muted)">PAR</div><div style="font-size:12px">{meta.get('uploaded_by','—')}</div></div>
  </div>
  <div class="card-sm" style="margin-bottom:14px">
    <div style="font-size:11px;color:var(--muted);margin-bottom:6px">CHANGELOG</div>
    <pre style="font-size:12px;color:var(--text2);white-space:pre-wrap;margin:0;font-family:'Segoe UI',sans-serif;line-height:1.7">{cl}</pre>
  </div>
  <div style="display:flex;gap:8px;flex-wrap:wrap">
    <a href="{meta.get('github_url','#')}" target="_blank" class="btn btn-sm btn-blue">⬇ Télécharger</a>
    <form method="POST" style="display:inline" onsubmit="return confirm('Supprimer cette update ?')">
      <input type="hidden" name="action" value="delete">
      <input type="hidden" name="is_beta_del" value="{del_flag}">
      <button type="submit" class="btn btn-sm btn-red">🗑 Supprimer</button>
    </form>
    <button onclick="navigator.clipboard.writeText('{meta.get('github_url','')}').then(()=>this.textContent='✅ Copié!')" class="btn btn-sm btn-ghost">📋 Copier URL</button>
  </div>
</div>"""

    stable_block  = _meta_block(meta_stable, "🟢 Canal STABLE", "var(--green)", "0")
    beta_block    = _meta_block(meta_beta,   "🟡 Canal BÊTA",   "var(--gold)", "1")

    # Historique des versions
    hist_rows = ""
    for h in history[:20]:
        canal_b  = '<span style="color:var(--gold);font-size:10px">BÊTA</span>' if h.get("is_beta") else '<span style="color:var(--green);font-size:10px">STABLE</span>'
        size_h   = f"{h.get('size_bytes',0)/1024/1024:.1f} Mo" if h.get("size_bytes") else "—"
        date_h   = fmt_ts(h.get("uploaded_at")) if h.get("uploaded_at") else "—"
        cl_short = (h.get("changelog","") or "")[:80]
        hist_rows += f'<tr><td style="font-weight:700;color:var(--text)">v{h.get("version","?")}</td><td>{canal_b}</td><td style="font-size:11px;color:var(--text2)">{date_h}</td><td style="font-size:11px;color:var(--muted)">{size_h}</td><td style="font-size:11px;color:var(--text2);max-width:300px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">{cl_short}</td><td><a href="{h.get("github_url","#")}" target="_blank" class="btn btn-xs btn-ghost">⬇</a></td></tr>'

    history_block = f"""
<div class="card" style="margin-top:16px">
  <div class="card-title">📜 Historique des versions ({len(history)})</div>
  {'<div style="overflow-x:auto"><table class="table"><thead><tr><th>Version</th><th>Canal</th><th>Publié</th><th>Taille</th><th>Changelog</th><th></th></tr></thead><tbody>' + hist_rows + '</tbody></table></div>' if history else '<div style="color:var(--muted);text-align:center;padding:16px">Aucun historique</div>'}
</div>""" if history else ""
...
<!-- COLONNE GAUCHE : Publier -->
<div>
  <div class="card">
    <div class="card-title">📤 Publier une update</div>
    <p style="font-size:12px;color:var(--text2);margin-bottom:14px;line-height:1.7">
      Upload sur <b>GitHub Releases</b>, puis colle l'URL directe ici.<br>
      Les clients téléchargent automatiquement à la prochaine connexion.
    </p>
    <form method="POST">
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px">
        <div class="form-group" style="margin:0">
          ...
          <input name="version" placeholder="7.2" required>
        </div>
        <div class="form-group" style="margin:0">
          <label>Taille estimée (Mo)</label>
          <input name="size_mb" placeholder="45.5" type="number" step="0.1" min="0">
        </div>
      </div>
      <div class="form-group" style="margin-top:10px">
        <label>URL directe .exe <span style="color:var(--red)">*</span></label>
        <input name="github_url" placeholder="https://github.com/…/releases/download/v7.2/WinOptimizer.exe" required style="width:100%">
        <div style="font-size:11px;color:var(--muted);margin-top:4px">
          💡 GitHub → Releases → New Release → Upload .exe → Copier le lien direct
        </div>
      </div>
      <div class="form-group" style="margin-top:10px">
        <label>Changelog</label>
        <textarea name="changelog" rows="5" placeholder="✓ Fix bug X&#10;✓ Amélioration Y&#10;✓ Nouvelle feature Z" style="width:100%;background:var(--card2);border:1px solid var(--border);border-radius:8px;padding:10px;color:var(--text);font-size:13px;resize:vertical;outline:none;font-family:'Segoe UI',sans-serif;line-height:1.6"></textarea>
      </div>
      <div style="display:flex;align-items:center;gap:12px;margin-top:12px">
        <label style="display:flex;align-items:center;gap:6px;cursor:pointer;font-size:13px">
          <input type="checkbox" name="is_beta" value="1" style="accent-color:var(--gold)">
          <span style="color:var(--gold)">🟡 Canal Bêta (test)</span>
        </label>
        <button type="submit" class="btn btn-green" style="margin-left:auto">⬆️ Publier</button>
      </div>
    </form>
  </div>
  ...
  <div class="card" style="margin-top:16px">
    ...
    <div style="margin-top:10px;background:var(--card2);border-radius:8px;padding:10px;font-size:11px">
      ...
      <code style="color:var(--green)">{{"version": "7.0", "beta": false}}</code>
    </div>
  </div>
</div>

<!-- COLONNE DROITE : Status actuel -->
<div>
  {stable_block}
  <div style="margin-top:12px">{beta_block}</div>
</div>
</div>
...
:root{--bg:#0d0d0f;--card:#1a1a1f;--card2:#1f1f26;--border:#2a2a3a;
  ...
  --gold:#ffd700;--purple:#7c5cbf;--muted:#444458}
body{background:var(--bg);color:var(--text);font-family:'Segoe UI',sans-serif;min-height:100vh}
a{text-decoration:none;color:inherit}
.nav{background:rgba(13,13,15,.95);backdrop-filter:blur(12px);border-bottom:1px solid var(--border);
  position:sticky;top:0;z-index:100;padding:0 5%}
.nav-inner{display:flex;align-items:center;justify-content:space-between;height:64px}
.logo{font-size:20px;font-weight:900;background:linear-gradient(135deg,var(--accent),var(--purple));
...
.hero{padding:80px 5% 60px;text-align:center}
.hero h1{font-size:clamp(32px,6vw,64px);font-weight:900;line-height:1.1;margin-bottom:20px}
...
.section-title{font-size:clamp(22px,4vw,36px);font-weight:800;text-align:center;margin-bottom:12px}
.section-sub{color:var(--text2);text-align:center;margin-bottom:48px;font-size:16px}
...
.plan-card{background:var(--card);border:2px solid var(--border);border-radius:20px;
  padding:36px;text-align:center;transition:.3s;position:relative}
.plan-card.popular{border-color:var(--accent);background:linear-gradient(180deg,rgba(79,142,247,.06),var(--card))}
...
.plan-price{font-size:48px;font-weight:900;color:var(--accent);margin:16px 0}
.plan-price sup{font-size:20px;vertical-align:top;margin-top:12px;display:inline-block}
...
.plan-features li .chk{color:var(--green);font-size:16px}
.plan-features li .x{color:var(--red)}
.diff-table{width:100%;border-collapse:collapse;font-size:14px}
.diff-table th{background:var(--card2);padding:14px;text-align:left;border-bottom:2px solid var(--border)}
.diff-table td{padding:12px 14px;border-bottom:1px solid var(--border)}
.diff-table tr:hover td{background:rgba(79,142,247,.04)}
.badge-yes{color:var(--green);font-weight:700}
.badge-no{color:var(--muted)}
footer{background:var(--card);border-top:1px solid var(--border);padding:40px 5%;
...
def _site_nav(active="home"):
    ...
    for label, href, key in links:
        style = "color:var(--text);font-weight:600" if active == key else ""
        items += f'<a href="{href}" style="{style}">{label}</a>'
    return f"""
<nav class="nav">
  <div class="nav-inner">
    <a href="/" class="logo">⚡ WinOptimizer Pro</a>
    <div class="nav-links">{items}</div>
    <div style="display:flex;gap:10px">
      <a href="/client" class="btn btn-outline" style="padding:8px 18px">Connexion</a>
      <a href="/pricing" class="btn btn-primary" style="padding:8px 18px">Acheter</a>
    </div>
  </div>
</nav>"""
...
.faq-q{{padding:16px 20px;cursor:pointer;font-weight:600;display:flex;justify-content:space-between;align-items:center;transition:.2s}}
.faq-q:hover{{background:rgba(79,142,247,.06)}}
.faq-arrow{{transition:.3s;color:var(--accent)}}
.faq-a{{padding:0 20px;max-height:0;overflow:hidden;transition:.4s;color:var(--text2);font-size:14px;line-height:1.7}}
...
<section class="section" style="background:var(--card2)">
  <div style="display:flex;justify-content:center;gap:48px;flex-wrap:wrap">
    <div style="text-align:center"><div style="font-size:36px;font-weight:900;color:var(--accent)">{s1v}</div><div style="color:var(--text2);font-size:14px">{s1l}</div></div>
    <div style="text-align:center"><div style="font-size:36px;font-weight:900;color:var(--green)">{s2v}</div><div style="color:var(--text2);font-size:14px">{s2l}</div></div>
    <div style="text-align:center"><div style="font-size:36px;font-weight:900;color:var(--gold)">{s3v}</div><div style="color:var(--text2);font-size:14px">{s3l}</div></div>
    <div style="text-align:center"><div style="font-size:36px;font-weight:900;color:var(--purple)">{s4v}</div><div style="color:var(--text2);font-size:14px">{s4l}</div></div>
  </div>
</section>
...
<section class="section">
  ...
  <div class="grid-2" style="max-width:800px;margin:0 auto">
    ...
    <div class="plan-card">
      ...
      <div class="plan-price"><sup>€</sup>{SHOP_PRICES['NORMAL']['price']:.2f}</div>
      <div style="color:var(--text2);font-size:14px;margin-bottom:20px">Paiement unique</div>
      <ul class="plan-features">
        <li><span class="chk">✓</span>+50 optimisations essentielles</li>
        <li><span class="chk">✓</span>Gaming Pack 25+ jeux</li>
        <li><span class="chk">✓</span>Réseau & confidentialité</li>
        <li><span class="chk">✓</span>Espace client inclus</li>
        <li><span class="x">✗</span>Tweaks CPU/GPU avancés</li>
        <li><span class="x">✗</span>Addons gaming</li>
      </ul>
      {'<a href="' + get_site_setting("discord_invite", DISCORD_INVITE) + '" target="_blank" class="btn btn-outline" style="width:100%;padding:14px;text-align:center">💬 Acheter via Discord</a>' if normal_active and stock_n > 0 else '<div style="color:var(--muted);font-size:14px;padding:12px">Indisponible</div>'}
    </div>

    <div class="plan-card popular">
      <div class="plan-badge">⭐ LE PLUS POPULAIRE</div>
      <div class="plan-name">⭐ PRO</div>
      <div class="plan-price"><sup>€</sup>{SHOP_PRICES['PRO']['price']:.2f}</div>
      <div style="color:var(--text2);font-size:14px;margin-bottom:20px">Paiement unique</div>
      <ul class="plan-features">
        <li><span class="chk">✓</span>Tout le plan NORMAL</li>
        <li><span class="chk">✓</span>+47 optimisations avancées</li>
        <li><span class="chk">✓</span>Tweaks CPU, HPET, Core Parking</li>
        <li><span class="chk">✓</span>Overclock CPU/GPU sécurisé</li>
        <li><span class="chk">✓</span>Presets Ultra & Streamer</li>
        <li><span class="chk">✓</span>6 addons gaming gratuits</li>
        <li><span class="chk">✓</span>Support prioritaire</li>
      </ul>
      {'<a href="' + get_site_setting("discord_invite", DISCORD_INVITE) + '" target="_blank" class="btn btn-primary" style="width:100%;padding:14px;text-align:center">💬 Acheter via Discord</a>' if pro_active and stock_p > 0 else '<div style="color:var(--muted);font-size:14px;padding:12px">Indisponible</div>'}
    </div>
  </div>
...
def site_client():
    ...
    # Historique de connexions (5 dernières)
    logins = conn.execute(
        "SELECT ip,ts,country,city,flag,success FROM login_history WHERE username=? ORDER BY ts DESC LIMIT 5",
        (username,)).fetchall()
    conn.close()
    ...
    # Bannière version/update
    meta_stable = _get_update_meta(beta=False)
    server_ver  = meta_stable.get("version") or ""
    update_banner = ""
    if server_ver and exe_url:
        update_banner = f"""
<div style="background:linear-gradient(135deg,rgba(0,255,136,.08),rgba(79,142,247,.08));border:1px solid rgba(0,255,136,.25);border-radius:12px;padding:16px;margin-bottom:16px;display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:12px">
  <div>
    <div style="font-size:13px;font-weight:700;color:var(--green)">⚡ {site_title} v{server_ver} disponible</div>
    <div style="font-size:12px;color:var(--text2);margin-top:2px">Télécharge la dernière version pour profiter de toutes les améliorations.</div>
  </div>
  <a href="{exe_url}" class="ec-btn ec-btn-primary" style="white-space:nowrap;padding:10px 20px">📥 Télécharger v{server_ver}</a>
</div>"""
    ...
    hw_rows = "".join(
        f'<div style="display:flex;gap:8px;padding:7px 0;border-bottom:1px solid var(--border)"><span style="min-width:90px;font-size:11px;color:var(--muted)">{k}</span><span style="font-size:12px;color:var(--text2);word-break:break-all">{v}</span></div>'
        for k, v in hw_items
    )
    ...
    login_rows = ""
    for lg in logins:
        ts_str   = datetime.fromtimestamp(lg["ts"]).strftime("%d/%m %H:%M") if lg["ts"] else "—"
        ok_icon  = "✅" if lg["success"] else "❌"
        loc      = f'{lg["flag"] or ""} {lg["city"] or ""}, {lg["country"] or ""}'.strip(" ,")
        login_rows += f'<div style="display:flex;align-items:center;gap:10px;padding:7px 0;border-bottom:1px solid var(--border);font-size:12px"><span>{ok_icon}</span><span style="color:var(--text2);min-width:90px">{ts_str}</span><code style="color:var(--accent);font-size:11px">{lg["ip"] or "—"}</code><span style="color:var(--muted)">{loc}</span></div>'
    if not login_rows:
        login_rows = '<div style="color:var(--muted);font-size:12px;padding:10px 0">Aucun historique disponible</div>'

    # Badge plan PRO
    plan_style = 'background:linear-gradient(135deg,rgba(255,215,0,.15),rgba(124,92,191,.15));border:1px solid rgba(255,215,0,.3);color:var(--gold)' if plan=="PRO" else 'background:rgba(79,142,247,.1);border:1px solid rgba(79,142,247,.2);color:var(--accent)'
    plan_label = '⭐ Plan PRO' if plan=="PRO" else '🔷 Plan NORMAL'
...
<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(130px,1fr));gap:10px;margin-bottom:18px">
  <div class="ec-stat">
    <div style="font-size:26px;font-weight:900;color:var(--accent)">{user['connections'] or 0}</div>
    <div style="font-size:11px;color:var(--text2)">Connexions</div>
  </div>
  <div class="ec-stat">
    <div style="font-size:26px;font-weight:900;color:var(--gold)">{tickets_open}</div>
    <div style="font-size:11px;color:var(--text2)">Tickets ouverts</div>
  </div>
  <div class="ec-stat">
    <div style="font-size:26px;font-weight:900;color:var(--green)">{tickets_ans}</div>
    <div style="font-size:11px;color:var(--text2)">Tickets répondus</div>
  </div>
  <div class="ec-stat">
    <div style="font-size:13px;font-weight:700;color:var(--text)">{created_str}</div>
    <div style="font-size:11px;color:var(--text2)">Membre depuis</div>
  </div>
  <div class="ec-stat">
    <span style="display:inline-block;padding:4px 12px;border-radius:20px;font-size:12px;font-weight:700;{plan_style}">{plan_label}</span>
    <div style="font-size:11px;color:var(--text2);margin-top:4px">Licence</div>
  </div>
</div>
...
<div style="display:grid;grid-template-columns:1fr 1fr;gap:14px;margin-bottom:14px">
  ...
  <div class="ec-card">
    ...
    {'<a href="'+exe_url+'" class="ec-btn ec-btn-primary" style="width:100%;text-align:center;display:block;padding:13px;font-size:14px">📥 Télécharger la dernière version</a>' if exe_url else '<div style="text-align:center;color:var(--text2);padding:16px;font-size:13px">Lien bientôt disponible — rejoins le <a href="'+discord_link+'" target="_blank" style="color:var(--accent)">Discord</a>.</div>'}
    <div style="margin-top:10px;font-size:11px;color:var(--muted);text-align:center">
      Dernière activité : {last_login_str}
    </div>
  </div>
  ...
  <div class="ec-card">
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px">
      <a href="/client/licence" class="ec-btn ec-btn-outline" style="text-align:center;padding:10px;font-size:12px">🔑 Ma licence</a>
      <a href="/client/tickets" class="ec-btn ec-btn-outline" style="text-align:center;padding:10px;font-size:12px">📩 Support</a>
      <a href="{discord_link}" target="_blank" class="ec-btn ec-btn-outline" style="text-align:center;padding:10px;font-size:12px">💬 Discord</a>
      <a href="/client/password" class="ec-btn ec-btn-outline" style="text-align:center;padding:10px;font-size:12px">🔒 Mot de passe</a>
    </div>
  </div>
</div>
...
<div style="display:grid;grid-template-columns:1fr 1fr;gap:14px">
  ...
  <!-- Hardware -->
  <div class="ec-card">
    <div class="ec-card-title">🖥 Ma configuration</div>
    <div style="padding:4px 0">{hw_rows}</div>
    <div style="font-size:10px;color:var(--muted);margin-top:10px">
      Enregistrée à la connexion — mise à jour automatique.
    </div>
  </div>

  <!-- Historique connexions -->
  <div class="ec-card">
    <div class="ec-card-title">🔐 Historique de connexions</div>
    <div>{login_rows}</div>
    <div style="font-size:11px;color:var(--muted);margin-top:10px">
      Si tu vois une connexion suspecte, <a href="/client/tickets" style="color:var(--accent)">contacte le support</a>.
    </div>
  </div>
</div>
...
def admin_site():
    ...
    try:
        ...
        chat_sessions = conn.execute("SELECT * FROM chatbot_sessions ORDER BY ts DESC LIMIT 30").fetchall()
        # Visits last 7 days
