...
def admin_dashboard():
    ...
    stats = {
        ...
        "lifetime":      conn.execute("SELECT COUNT(*) FROM users WHERE plan='LIFETIME'").fetchone()[0],
...
def admin_user_action(username):
    ...
    if action == "suspend":
        ...
        conn.execute("UPDATE users SET plan='LIFETIME' WHERE username=?", (username,))
        add_log("OK","ADMIN",f"Upgrade LIFETIME: {username}", session["admin_user"])
...
def admin_addons_generate():
    ...
    for _ in range(qty):
        ...
        conn.execute("INSERT OR IGNORE INTO addon_licenses (key,addon_key,plan,status,created_at,note) VALUES (?,?,?,?,?,?)",
                     (key, addon, "LIFETIME", "active", int(time.time()), note))
...
def internal_broadcast():
    ...
    q = {
        ...
        "lifetime": "SELECT discord_id FROM users WHERE plan='LIFETIME' AND status='active' AND discord_id IS NOT NULL AND discord_id!=''",
...
def internal_gen_addon_key():
    ...
    for _ in range(qty):
        key = generate_addon_key(addon)
        conn.execute("INSERT OR IGNORE INTO addon_licenses (key,addon_key,plan,status,created_at,note) VALUES (?,?,?,?,?,?)",
                     (key, addon, "LIFETIME", "active", int(time.time()), "Généré via bot Discord"))
        keys.append(key)
    conn.commit(); conn.close()
...
def internal_upgrade_plan():
    ...
    if plan not in ("NORMAL","PRO","LIFETIME"):
        return jsonify({"error":"Plan invalide — utilise NORMAL, PRO ou LIFETIME"}), 400
    conn = get_db()
...
def _page_dashboard(stats, recent_users, recent_logs, top_ips, admin_user, admin_role):
    ...
    for u in recent_users:
        if u["plan"]=="LIFETIME": pb='<span class="badge b-purple">♾️ LIFETIME</span>'
        elif u["plan"]=="PRO": pb='<span class="badge b-blue">⭐ PRO</span>'
...
<div class="stats-grid">
  ...
  <div class="stat-card"><div class="stat-top"><div class="stat-icon">♾️</div></div><div class="stat-num" id="stat-lifetime" style="color:var(--purple)">{stats['lifetime']}</div><div class="stat-label">Plan LIFETIME</div></div>
...
<div class="search-wrap">
  <form method="GET" style="display:contents">
    ...
    <select name="plan" style="max-width:140px"><option value="">Tous plans</option><option {'selected' if plan_f=='LIFETIME' else ''} value="LIFETIME">♾️ LIFETIME</option><option {'selected' if plan_f=='PRO' else ''} value="PRO">⭐ PRO</option><option {'selected' if plan_f=='NORMAL' else ''} value="NORMAL">NORMAL</option></select>
...
def _page_user_detail(user, logs, alerts, login_hist, notes, remote_hist, geo, msg, admin_user, admin_role):
    ...
    if user["plan"]=="LIFETIME": pb='<span class="badge b-purple">♾️ LIFETIME</span>'
    elif user["plan"]=="PRO": pb='<span class="badge b-blue">⭐ PRO</span>'
    else: pb='<span class="badge b-gray">NORMAL</span>'
...
<div class="card">
  ...
  <div style="display:flex;flex-wrap:wrap;gap:6px">
    <form method="POST" action="/admin/user/{user['username']}/action" style="display:contents">
      ...
      <button name="action" value="upgrade_lifetime" class="btn btn-sm btn-purple">♾️ LIFETIME</button>
...
def _page_keys(keys, addon_keys, admin_user, admin_role):
    ...
    for k in keys:
        ...
        if k["plan"]=="LIFETIME": pb='<span class="badge b-purple">♾️ LIFETIME</span>'
        elif k["plan"]=="PRO": pb='<span class="badge b-blue">⭐ PRO</span>'
        else: pb='<span class="badge b-gray">NORMAL</span>'
...
<div class="card">
  ...
  <form method="POST" action="/admin/keys/generate">
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px">
      <div class="form-group" style="margin:0"><label>Plan</label><select name="plan"><option value="NORMAL">NORMAL</option><option value="PRO">⭐ PRO</option><option value="LIFETIME">♾️ LIFETIME (à vie)</option></select></div>
