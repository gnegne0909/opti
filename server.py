...
SMTP_FROM    = os.environ.get("SMTP_FROM", "WinOptimizer Pro <noreply@winoptimizer.pro>")
...
def api_support_frame():
    ...
    with _remote_lock:
        ...
        # Garder seulement les 5 dernières frames
        sess["frames"] = sess["frames"][-4:] + [frame_b64]
...
def _get_update_meta():
    try:
        ...
        return {"version": None, "filename": None, "github_url": None, "uploaded_at": None, "uploaded_by": None, "changelog": ""}
...
def admin_update():
    ...
    if request.method == "POST":
        ...
        if action == "upload":
            ...
            if not version:
            ...
            elif not github_url or not github_url.startswith("https://github.com/gnegne0909/.exe/releases/download/"):
                msg = "❌ URL GitHub invalide. Format attendu : https://github.com/gnegne0909/.exe/releases/download/vX.X/fichier.exe"
            else:
                ...
                add_log("OK", "UPDATE", f"Update v{version} publiée par {session['admin_user']} → {github_url}", session["admin_user"])
                _push_sse({"type": "update_pushed", "version": version, "admin": session["admin_user"]})
                msg = f"✅ Update v{version} ({filename}) publiée avec succès !"
        elif action == "delete":
            meta = _get_update_meta()
...
<div>
  <div class="card">
    ...
    <form method="POST" id="upload-form">
      ...
      <div class="form-group" style="margin-top:10px">
        <label style="font-size:12px;color:var(--text2)">URL GitHub Release (lien direct .exe)</label>
        <input name="github_url" placeholder="https://github.com/gnegne0909/.exe/releases/download/v7.1/WinOptimizer.exe" required style="margin-top:4px;width:100%;background:var(--card2);border:1px solid var(--border);border-radius:8px;padding:10px;color:var(--text);font-size:12px;outline:none">
        <div style="font-size:11px;color:var(--muted);margin-top:6px">
          📌 Va sur <b style="color:var(--green)">github.com/gnegne0909/.exe → Releases</b>, crée une release, upload ton .exe, et colle ici le lien direct.
        </div>
      </div>
      <button type="submit" class="btn btn-green" style="margin-top:10px">⬆️ Publier l'update</button>
    </form>
  </div>
...
def _send_email(to_email: str, subject: str, html_body: str):
    """Envoie un email via SMTP. Silencieux si non configuré."""
...
def _site_footer():
    return f"""
<footer>
  <div style="font-size:22px;font-weight:900;background:linear-gradient(135deg,#4f8ef7,#7c5cbf);-webkit-background-clip:text;-webkit-text-fill-color:transparent">⚡ WinOptimizer Pro</div>
  <div class="links">
    <a href="/">Accueil</a>
  ...
  <div style="color:var(--text2);font-size:12px">© 2025 WinOptimizer Pro — Tous droits réservés</div>
</footer>"""
...
<title>WinOptimizer Pro — Optimise ton Windows</title>{SITE_CSS}</head><body>
{_site_nav("home")}
<section class="hero">
  ...
  <h1>Optimise ton Windows<br><span>pour des FPS max</span></h1>
  <p>Le logiciel d'optimisation Windows le plus avancé pour les gamers. +97 tweaks système, gaming et réseau.</p>
  <div class="hero-btns">
...
<section class="section" style="background:var(--card2)">
  <div style="display:flex;justify-content:center;gap:48px;flex-wrap:wrap">
    <div style="text-align:center"><div style="font-size:36px;font-weight:900;color:var(--accent)">+97</div>
    <div style="color:var(--text2);font-size:14px">Optimisations</div></div>
    <div style="text-align:center"><div style="font-size:36px;font-weight:900;color:var(--green)">+25</div>
    <div style="color:var(--text2);font-size:14px">Jeux supportés</div></div>
    <div style="text-align:center"><div style="font-size:36px;font-weight:900;color:var(--gold)">-30ms</div>
    <div style="color:var(--text2);font-size:14px">Latence réseau</div></div>
    <div style="text-align:center"><div style="font-size:36px;font-weight:900;color:var(--purple)">+15%</div>
    <div style="color:var(--text2);font-size:14px">FPS moyens</div></div>
  </div>
</section>
...
<section class="section" style="background:var(--card2);text-align:center">
  ...
  <div class="section-sub">Rejoins des milliers de gamers qui font confiance à WinOptimizer Pro</div>
  <div style="display:flex;gap:12px;justify-content:center;flex-wrap:wrap">
    <a href="/pricing" class="btn btn-primary" style="padding:14px 36px;font-size:16px">🛍️ Acheter maintenant</a>
    <a href="{DISCORD_INVITE}" target="_blank" class="btn btn-outline" style="padding:14px 36px;font-size:16px">💬 Rejoindre Discord</a>
  </div>
</section>
...
<section class="section">
  ...
  <div class="grid-2" style="max-width:800px;margin:0 auto">
    ...
    <div class="plan-card">
      ...
      {'<button onclick="openCheckout(\'NORMAL\')" class="btn btn-outline" style="width:100%;padding:14px">Choisir NORMAL</button>' if normal_active and stock_n > 0 else '<div style="color:var(--muted);font-size:14px;padding:12px">Indisponible</div>'}
    </div>
    ...
    <div class="plan-card popular">
      ...
      {'<button onclick="openCheckout(\'PRO\')" class="btn btn-primary" style="width:100%;padding:14px">Choisir PRO</button>' if pro_active and stock_p > 0 else '<div style="color:var(--muted);font-size:14px;padding:12px">Indisponible</div>'}
    </div>
  </div>
...
<div class="modal-bg" id="checkout-modal">
  <div class="modal">
    <h2 id="checkout-title">Commander WinOptimizer Pro</h2>
    <div id="checkout-msg"></div>
    <form id="checkout-form">
      <input type="hidden" id="checkout-plan" name="plan">
      <input type="hidden" id="checkout-price" name="price">
      <div class="form-group">
        <label>Adresse email</label>
        <input type="email" name="email" id="checkout-email" placeholder="ton@email.com" required>
      </div>
      <div class="form-group">
        <label>Code promo (optionnel)</label>
        <input type="text" name="promo" id="checkout-promo" placeholder="CODE10">
      </div>
      <div id="price-display" style="background:var(--card2);border-radius:8px;padding:14px;margin-bottom:16px;text-align:center">
        <span style="font-size:24px;font-weight:900;color:var(--accent)" id="price-val"></span>
        <span style="color:var(--text2);font-size:14px"> — paiement unique</span>
      </div>
      <div class="form-group">
        <label>Mode de paiement</label>
        <select name="payment_method" id="checkout-method">
          <option value="paypal">💳 PayPal</option>
          <option value="paypal_friends">🤝 PayPal Amis/Proches</option>
          <option value="card">💳 Carte bancaire</option>
        </select>
      </div>
      <div id="payment-instructions" style="background:rgba(79,142,247,.08);border:1px solid rgba(79,142,247,.2);
        border-radius:8px;padding:14px;margin-bottom:16px;font-size:13px;color:var(--text2)">
      </div>
      <button type="submit" class="btn btn-primary" style="width:100%;padding:14px;font-size:16px">
        🔐 Confirmer la commande
      </button>
      <button type="button" onclick="closeCheckout()" class="btn btn-outline" 
        style="width:100%;padding:10px;margin-top:8px;font-size:14px">Annuler</button>
    </form>
  </div>
</div>

<script>
var PRICES = {{"NORMAL":{SHOP_PRICES['NORMAL']['price']}, "PRO":{SHOP_PRICES['PRO']['price']}}};
var currentPlan = "";
function openCheckout(plan) {{
  currentPlan = plan;
  document.getElementById("checkout-plan").value = plan;
  document.getElementById("checkout-title").textContent = "Commander — " + plan;
  document.getElementById("price-val").textContent = PRICES[plan].toFixed(2) + "€";
  document.getElementById("checkout-price").value = PRICES[plan];
...
function updatePaymentInstructions() {{
  var method = document.getElementById("checkout-method").value;
  var inst = document.getElementById("payment-instructions");
  var msgs = {{
    "paypal": "💳 <b>PayPal standard</b> : tu recevras un lien de paiement PayPal par email après confirmation.",
    "paypal_friends": "🤝 <b>PayPal Amis/Proches</b> : envoie le montant à notre email PayPal (communiqué après commande). Ton compte sera activé sous 24h.",
    "card": "💳 <b>Carte bancaire</b> : tu seras redirigé vers notre page de paiement sécurisée."
  }};
  inst.innerHTML = msgs[method] || "";
}}
document.getElementById("checkout-method").addEventListener("change", updatePaymentInstructions);
document.getElementById("checkout-promo").addEventListener("blur", async function() {{
  var code = this.value.trim().toUpperCase();
  if (!code) return;
  var r = await fetch("/api/shop/check_promo?code=" + code + "&plan=" + currentPlan);
  var d = await r.json();
  if (d.valid) {{
    var newPrice = (PRICES[currentPlan] * (1 - d.reduction/100)).toFixed(2);
    document.getElementById("price-val").textContent = newPrice + "€ (-" + d.reduction + "%)";
    document.getElementById("checkout-price").value = newPrice;
  }}
}});
document.getElementById("checkout-form").addEventListener("submit", async function(e) {{
  ...
  var r = await fetch("/api/shop/order", {{method:"POST",headers:{{"Content-Type":"application/json"}},body:JSON.stringify(data)}});
  var d = await r.json();
  var msgDiv = document.getElementById("checkout-msg");
  if (d.success) {{
    msgDiv.innerHTML = '<div class="msg-ok">✅ ' + d.message + '</div>';
    document.getElementById("checkout-form").style.display = "none";
  }} else {{
    msgDiv.innerHTML = '<div class="msg-err">❌ ' + (d.error || d.reason || "Erreur") + '</div>';
    btn.textContent = "🔐 Confirmer la commande"; btn.disabled = false;
  }}
}});
</script>
{_site_footer()}
...
def _site_maintenance_page():
    return render_template_string(f"""<!DOCTYPE html><html lang="fr"><head>
<meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>Maintenance — WinOptimizer Pro</title>{SITE_CSS}</head><body>
<div style="display:flex;align-items:center;justify-content:center;min-height:100vh;text-align:center;padding:20px">
  <div>
    ...
    <h1 style="font-size:32px;font-weight:800;margin-bottom:12px">Site en maintenance</h1>
    <p style="color:var(--text2);margin-bottom:24px">Nous revenons très bientôt. Merci de ta patience !</p>
    <a href="{DISCORD_INVITE}" target="_blank" class="btn btn-primary">💬 Discord</a>
  </div>
</div></body></html>""")
...
def site_client():
    ...
    conn = get_db()
    orders = conn.execute(
        "SELECT * FROM shop_orders WHERE email=? AND status='completed' ORDER BY created_at DESC",
        (client_email,)
    ).fetchall()
    conn.close()
    ...
    for o in orders:
        plan = o["plan"]
        date_str = datetime.fromtimestamp(o["created_at"]).strftime("%d/%m/%Y")
        exe_url = get_site_setting(f"exe_url_{plan.lower()}", "")
        orders_html += f"""
<div class="card" style="margin-bottom:16px">
  <div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:12px">
    <div>
      <div style="font-weight:700;font-size:16px">{'⭐ PRO' if plan=='PRO' else '🔷 NORMAL'}</div>
      <div style="color:var(--text2);font-size:13px">Commande #{o['order_id']} — {date_str}</div>
    </div>
    <div style="display:flex;gap:8px;flex-wrap:wrap">
      {'<a href="' + exe_url + '" class="btn btn-primary" style="padding:8px 18px;font-size:13px">📥 Télécharger</a>' if exe_url else '<span style="color:var(--text2);font-size:13px">Lien bientôt disponible</span>'}
      <a href="/client/invoice/{o['order_id']}" class="btn btn-outline" style="padding:8px 18px;font-size:13px">🧾 Facture</a>
    </div>
  </div>
  <div style="margin-top:14px;background:var(--card2);border-radius:8px;padding:12px;font-size:13px">
    <div style="color:var(--text2);margin-bottom:4px">Clé de licence</div>
    <div style="font-family:monospace;color:var(--accent);letter-spacing:1px">{o['license_key'] or '—'}</div>
  </div>
</div>"""

    if not orders_html:
        orders_html = '<div style="text-align:center;color:var(--text2);padding:40px">Aucun produit trouvé. <a href="/pricing" style="color:var(--accent)">Acheter une licence</a></div>'

    return render_template_string(f"""<!DOCTYPE html><html lang="fr"><head>
<meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>Espace Client — WinOptimizer Pro</title>{SITE_CSS}</head><body>
{_site_nav("client")}
<section class="section" style="max-width:800px;margin:0 auto">
  <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:32px;flex-wrap:wrap;gap:12px">
    <div>
      <h1 style="font-size:28px;font-weight:800">👤 Espace Client</h1>
      <div style="color:var(--text2);font-size:14px">{client_email}</div>
    </div>
    <a href="/client/logout" class="btn btn-outline" style="padding:8px 18px;font-size:13px">Déconnexion</a>
  </div>
  <h2 style="font-size:18px;font-weight:700;margin-bottom:16px">📦 Mes produits</h2>
  {orders_html}
  <div style="margin-top:24px;text-align:center">
    <a href="/pricing" class="btn btn-primary" style="padding:12px 28px">🛍️ Acheter une autre licence</a>
  </div>
</section>
{_site_footer()}
</body></html>""")
...
def site_client_login():
    ...
    if request.method == "POST":
        ...
        if action == "send_otp":
            if not email or "@" not in email:
                ...
                # Vérifier si l'email existe dans les commandes
                conn = get_db()
                exists = conn.execute("SELECT 1 FROM shop_orders WHERE email=? AND status='completed'", (email,)).fetchone()
                conn.close()
                if not exists:
                    ...
                    otp = _gen_otp()
                    conn = get_db()
                    conn.execute("DELETE FROM client_sessions WHERE email=?", (email,))
                    conn.execute("INSERT INTO client_sessions (email,otp,created_at) VALUES (?,?,?)",
                                 (email, otp, int(time.time())))
                    conn.commit(); conn.close()
                    # Envoyer OTP par mail
                    sent = _send_email(email, "WinOptimizer Pro — Code de connexion",
                        f"""<div style="font-family:Arial,sans-serif;background:#0d0d0f;color:#fff;padding:40px">
                        <h2>⚡ Code de connexion</h2><p>Ton code temporaire est :</p>
                        <div style="font-size:48px;font-weight:900;color:#4f8ef7;letter-spacing:12px;text-align:center;padding:20px 0">{otp}</div>
                        <p style="color:#888">Ce code expire dans 10 minutes.</p></div>""")
                    msg = f"✅ Code envoyé à {email}" + (" (vérifiez vos spams)" if not sent else "")
                    msg_type = "ok"
                    session["pending_otp_email"] = email
        elif action == "verify_otp":
            otp_input = request.form.get("otp","").strip()
            pend_email = session.get("pending_otp_email","")
            conn = get_db()
            row = conn.execute(
                "SELECT * FROM client_sessions WHERE email=? AND otp=? AND used=0 AND created_at>?",
                (pend_email, otp_input, int(time.time())-600)
            ).fetchone()
            if row:
                conn.execute("UPDATE client_sessions SET used=1 WHERE id=?", (row["id"],))
                conn.commit(); conn.close()
                session["client_email"] = pend_email
    ...
    pending = session.get("pending_otp_email","")
    return render_template_string(f"""<!DOCTYPE html><html lang="fr"><head>
<meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>Connexion Client — WinOptimizer Pro</title>{SITE_CSS}</head><body>
{_site_nav("client")}
<section style="display:flex;align-items:center;justify-content:center;min-height:calc(100vh - 64px);padding:20px">
  <div style="background:var(--card);border:1px solid var(--border);border-radius:20px;padding:40px;max-width:420px;width:100%">
    <div style="text-align:center;margin-bottom:28px">
      ...
      <h1 style="font-size:22px;font-weight:800">Espace Client</h1>
      <p style="color:var(--text2);font-size:14px;margin-top:6px">Connecte-toi avec ton email d'achat</p>
    </div>
    ...
    {f"""<form method="POST">
      ...
      <div class="form-group">
        ...
        <input type="text" name="otp" placeholder="123456" maxlength="6" style="text-align:center;font-size:24px;letter-spacing:8px" autofocus required>
      </div>
      <button type="submit" class="btn btn-primary" style="width:100%;padding:14px">✅ Vérifier le code</button>
      <div style="text-align:center;margin-top:14px">
        <a href="/client/login" style="color:var(--text2);font-size:13px">← Utiliser un autre email</a>
      </div>
    </form>""" if pending else """<form method="POST">
      <input type="hidden" name="action" value="send_otp">
      <div class="form-group">
        <label>Adresse email d'achat</label>
        <input type="email" name="email" placeholder="ton@email.com" autofocus required>
      </div>
      <button type="submit" class="btn btn-primary" style="width:100%;padding:14px">📧 Recevoir mon code</button>
    </form>"""}
  </div>
</section>
{_site_footer()}
...
def site_client_invoice(order_id):
    ...
    if not client_email:
        return redirect("/client/login")
    conn = get_db()
    order = conn.execute("SELECT * FROM shop_orders WHERE order_id=? AND email=?",
                          (order_id, client_email)).fetchone()
    conn.close()
    if not order:
        return redirect("/client")
...
def api_shop_order():
    ...
    key_res_data = {"plan": plan, "qty": 1, "bot_token": BOT_TOKEN}
    conn2 = get_db()
    key_row = conn2.execute("SELECT key FROM licenses WHERE status='active' AND plan=? AND key NOT IN (SELECT license_key FROM users WHERE license_key IS NOT NULL) LIMIT 1", (plan,)).fetchone()
    if key_row:
        license_key = key_row["key"]
        conn2.execute("UPDATE licenses SET status='sold' WHERE key=?", (license_key,))
    else:
        # Générer une nouvelle clé
        license_key = generate_key(plan)
        conn2.execute("INSERT INTO licenses (key,plan,status,created_at) VALUES (?,?,?,?)",
    ...
    conn.execute("""INSERT OR IGNORE INTO users
        (username,password_hash,license_key,plan,status,created_at,ip,first_login_done)
        VALUES (?,?,?,?,?,?,?,0)""",
        (username, password_hash, license_key, plan, "active", int(time.time()), ip))

    # Enregistrer la commande
    conn.execute("""INSERT INTO shop_orders
        (order_id,email,plan,price,status,license_key,username,ip,payment_method,created_at,promo_code)
        VALUES (?,?,?,?,?,?,?,?,?,?,?)""",
        (order_id, email, plan, price, "pending" if method in ("paypal_friends","card") else "pending",
         license_key, username, ip, method, int(time.time()), promo or None))

    # Décrémenter le stock
    set_site_setting(f"stock_{plan.lower()}", str(max(0, stock-1)))
    conn.commit(); conn.close()

    # Email de bienvenue + facture
    threading.Thread(target=lambda: _send_email(
        email,
...
def admin_site():
    ...
    orders_rows = ""
    for o in orders:
        status_color = {"completed":"var(--green)","pending":"var(--gold)","cancelled":"var(--red)"}.get(o["status"],"var(--text2)")
        orders_rows += f"""<tr>
          ...
          <td>
            <form method="POST" action="/admin/site/order/{o['order_id']}/complete" style="display:inline">
    ...
    for p in promos:
        promos_rows += f"""<tr>
          ...
          <td>
            <form method="POST" action="/admin/site/promo/{p['id']}/toggle" style="display:inline">
...
<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:12px;margin-bottom:20px">
  <div class="card"><div style="color:var(--text2);font-size:11px">COMMANDES</div><div style="font-size:28px;font-weight:800">{total_orders}</div></div>
  <div class="card"><div style="color:var(--text2);font-size:11px">EN ATTENTE</div><div style="font-size:28px;font-weight:800;color:var(--gold)">{pending_orders}</div></div>
  <div class="card"><div style="color:var(--text2);font-size:11px">REVENUS TOTAL</div><div style="font-size:28px;font-weight:800;color:var(--green)">{total_revenue:.2f}€</div></div>
</div>

<!-- Paramètres produits -->
<div class="card" style="margin-bottom:14px">
  <div class="card-title">⚙️ Paramètres produits & site</div>
  <form method="POST" action="/admin/site/settings">
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:14px">
      <div>
        <div class="form-group"><label>🔷 Lien .exe NORMAL (espace client)</label><input name="exe_url_normal" value="{settings['exe_url_normal']}" placeholder="https://..."></div>
        <div class="form-group"><label>Stock NORMAL</label><input name="stock_normal" value="{settings['stock_normal']}" type="number"></div>
        <div class="form-group"><label>Produit NORMAL actif</label>
          <select name="product_normal_active">
            <option value="1" {'selected' if settings['product_normal_active']=='1' else ''}>✅ Actif</option>
            <option value="0" {'selected' if settings['product_normal_active']=='0' else ''}>🔴 Désactivé</option>
          </select>
        </div>
      </div>
      <div>
        <div class="form-group"><label>⭐ Lien .exe PRO (espace client)</label><input name="exe_url_pro" value="{settings['exe_url_pro']}" placeholder="https://..."></div>
        <div class="form-group"><label>Stock PRO</label><input name="stock_pro" value="{settings['stock_pro']}" type="number"></div>
        <div class="form-group"><label>Produit PRO actif</label>
          <select name="product_pro_active">
            <option value="1" {'selected' if settings['product_pro_active']=='1' else ''}>✅ Actif</option>
            <option value="0" {'selected' if settings['product_pro_active']=='0' else ''}>🔴 Désactivé</option>
          </select>
        </div>
      </div>
    </div>
    <div class="form-group"><label>🔗 Lien Discord invite</label><input name="discord_invite" value="{settings['discord_invite']}"></div>
    <div class="form-group"><label>Site en maintenance</label>
      <select name="site_maintenance">
        <option value="0" {'selected' if settings['site_maintenance']=='0' else ''}>🟢 En ligne</option>
        <option value="1" {'selected' if settings['site_maintenance']=='1' else ''}>🔴 Maintenance</option>
      </select>
    </div>
    <button type="submit" class="btn btn-blue btn-sm">💾 Sauvegarder</button>
  </form>
...
<div class="card" style="margin-bottom:14px">
  ...
  <div style="overflow-x:auto">
    <table class="table">
      <thead><tr><th>N° Commande</th><th>Email</th><th>Plan</th><th>Prix</th><th>Statut</th><th>Date</th><th>Action</th></tr></thead>
      <tbody>{orders_rows if orders_rows else '<tr><td colspan="7" style="text-align:center;color:var(--text2)">Aucune commande</td></tr>'}</tbody>
    </table>
  </div>
</div>
...
<div class="card">
  ...
  <form method="POST" action="/admin/site/promo/create" style="display:flex;gap:10px;flex-wrap:wrap;margin-bottom:14px">
    <input name="code" placeholder="CODE" style="flex:1;min-width:100px">
    <input name="reduction" type="number" value="10" placeholder="%" style="width:70px">
    <select name="plan" style="width:100px">
      <option value="ALL">ALL</option><option value="NORMAL">NORMAL</option><option value="PRO">PRO</option>
    </select>
    <input name="max_uses" type="number" value="0" placeholder="Max" style="width:70px">
    <button type="submit" class="btn btn-blue btn-sm">Créer</button>
  </form>
  <table class="table">
    <thead><tr><th>Code</th><th>Réduction</th><th>Plan</th><th>Utilisations</th><th>Statut</th><th>Action</th></tr></thead>
    <tbody>{promos_rows if promos_rows else '<tr><td colspan="6" style="text-align:center;color:var(--text2)">Aucun code</td></tr>'}</tbody>
  </table>
</div>"""
...
def admin_site_settings():
    for key in ["exe_url_normal","exe_url_pro","stock_normal","stock_pro",
...
def admin_order_complete(order_id):
    ...
    conn.commit(); conn.close()
    add_log("OK","SHOP",f"Commande {order_id} marquée completed", session["admin_user"])
    return redirect("/admin/site")
...
def admin_promo_create():
    ...
    reduction= int(request.form.get("reduction",10))
