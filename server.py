...
def api_addon_activate():
    data = request.get_json(silent=True) or {}
    username   = data.get("username","").strip().lower()
    addon_key  = data.get("addon_key","").strip()
    license_key = data.get("license_key","").strip().upper()
    conn = get_db()
    row = conn.execute("SELECT * FROM addon_licenses WHERE key=? AND addon_key=?", (license_key, addon_key)).fetchone()
    if not row:
        conn.close(); return jsonify({"success": False, "reason": "Clé addon invalide ou mauvais addon"})
    if row["status"] != "active":
        conn.close(); return jsonify({"success": False, "reason": f"Clé {row['status']}"})
    conn.execute("UPDATE addon_licenses SET status='used' WHERE key=?", (license_key,))
    conn.commit(); conn.close()
    add_log("OK","ADDON",f"Addon {addon_key} activé par {username}", username)
    return jsonify({"success": True, "addon_key": addon_key})
  ...
  fetch('/admin/remote/'+SESSION_ID+'/connect',{{method:'POST'}})
    .then(r=>r.json()).then(d=>{{
      if(d.success){{
        ...
        startRemoteViewer(SESSION_ID);
        setupRemoteInput(SESSION_ID);
...
function typeText(){{
  var txt = document.getElementById('type-input').value;
  if(txt){{ sendRemoteCmd(SESSION_ID,'TYPE:'+txt); document.getElementById('type-input').value=''; showToast('Texte envoyé'); }}
}}
function takeScreenshot(){{
  var img = document.getElementById('remote-img');
  if(!img.src || img.style.display==='none'){{ showToast('Aucun frame disponible','yellow'); return; }}
  var a=document.createElement('a');a.href=img.src;a.download='screenshot_'+Date.now()+'.jpg';a.click();
  showToast('Screenshot sauvegardé');
}}

// FPS counter overlay
var _origStart = startRemoteViewer;
startRemoteViewer = function(sid){{
  _origStart(sid);
  setInterval(function(){{
    var now=Date.now();var dt=(now-_lastFpsTime)/1000;
    var el=document.getElementById('fps-counter');
    if(el)el.textContent='FPS: ~'+Math.round(_frameCount/dt);
    _frameCount=0;_lastFpsTime=now;
  }},2000);
}};

// Override frame display to count FPS
var _origFetch = window.fetch;
window.fetch = function(url,...args){{
  if(url && url.includes('/frame')){{
    return _origFetch(url,...args).then(r=>r.json()).then(d=>{{
      if(d.frame)_frameCount++;
      return d;
    }});
  }}
