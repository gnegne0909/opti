...
CORRECTIONS v2.2 :
  - Vraie IP via X-Forwarded-For / X-Real-IP (proxy Render/Nginx)
  - GeoIP via ip-api.com (gratuit, sans clé)
  - OS client stocké depuis le logiciel (plus "Windows" générique)
...
def get_real_ip():
    """
    Récupère la vraie IP du client même derrière Render/Nginx/Cloudflare.
    Ordre de priorité : CF-Connecting-IP > X-Real-IP > X-Forwarded-For > remote_addr
    """
    # Cloudflare
    ...
    # Proxies génériques — prendre la 1ère IP (l'originale du client)
    forwarded = request.headers.get("X-Forwarded-For")
    if forwarded:
        # Format : "client_ip, proxy1_ip, proxy2_ip"
        first_ip = forwarded.split(",")[0].strip()
        if first_ip and first_ip not in ("127.0.0.1", "::1"):
            return first_ip
...
def init_db():
    ...
    # Migration : ajouter os_info si la colonne n'existe pas encore
    try:
        c.execute("ALTER TABLE users ADD COLUMN os_info TEXT")
        conn.commit()
    except Exception:
        pass
...
def api_register():
    ...
    os_info     = data.get("os_info", "").strip()[:100]
    ip          = get_real_ip()
    ...
    conn.execute("""INSERT INTO users
        (username, password_hash, license_key, plan, discord_id, ip, os_info, status, created_at, first_login_done)
        VALUES (?,?,?,?,?,?,?,?,?,?)""",
        (username, ph, license_key, plan, discord_id, ip, os_info, "active", int(time.time()), 0))
    conn.commit()
    conn.close()

    add_log("OK", "REGISTER", f"Nouveau compte: {username} plan={plan} IP={ip} OS={os_info}", username)
...
def api_login():
    ...
    # Mettre à jour ip, os_info et stats de connexion
    conn.execute("""UPDATE users SET
        ...
        first_login_done=1
        WHERE username=?""", (int(time.time()), ip, os_info, username))
    conn.commit()
    conn.close()

    add_log("OK", "LOGIN", f"Connexion: {username} plan={row['plan']} IP={ip} OS={os_info}", username)
...
def admin_api_recent():
    conn = get_db()
    rows = conn.execute("SELECT username, plan, status, last_login, ip, connections, os_info FROM users ORDER BY last_login DESC LIMIT 10").fetchall()
    conn.close()
...
def admin_search():
    ...
    if q:
        ...
        results = conn.execute(
            "SELECT username, plan, status, discord_id, ip, last_login, connections, os_info FROM users WHERE username LIKE ? OR discord_id LIKE ? OR ip LIKE ? OR license_key LIKE ? ORDER BY last_login DESC LIMIT 50",
            (f"%{q}%", f"%{q}%", f"%{q}%", f"%{q}%")
        ).fetchall()
        conn.close()
...
.os-badge { display:inline-block; padding:2px 7px; background:#0a0020;
            border:1px solid #4422aa44; border-radius:3px; color:#8866cc; font-size:9px; }
</style>
"""
...
<div class="sidebar">
  <div class="sidebar-logo">
    ...
    <div class="sub">Panel d'administration v2.2</div>
  </div>
...
.user-row-mini { display:flex; align-items:center; gap:8px; padding:8px 0; border-bottom:1px solid #0a1a2a; flex-wrap:wrap; }
  ...
  fetch('/admin/api/recent_users').then(r=>r.json()).then(users=>{
    ...
    el.innerHTML = users.map(u=>`
      <div class="user-row-mini">
        ...
        ${u.os_info ? `<span class="os-badge">💻 ${u.os_info.substring(0,30)}</span>` : ''}
...
<div class="main">
  ...
  <div class="card" style="padding:0"><table>
    <thead><tr><th>Username</th><th>Plan</th><th>Statut</th><th>IP Réelle + Localisation</th><th>OS</th><th>Discord</th><th>Dernière connexion</th><th>Connexions</th><th>Actions</th></tr></thead>
    <tbody>{% for u in users %}<tr>
      <td><a href="/admin/user/{{ u.username }}">{{ u.username }}</a></td>
      <td><span class="badge {{ u.plan }}">{{ u.plan }}</span></td>
      <td><span class="badge {{ u.status }}">{{ u.status }}</span></td>
      <td>
        <span style="color:#00d4ff;font-size:10px;font-weight:bold">{{ u.ip or '—' }}</span>
        {% if u.ip %}
      ...
      <td>{% if u.os_info %}<span class="os-badge">{{ u.os_info[:35] }}</span>{% else %}<span style="color:#2a4a6a">—</span>{% endif %}</td>
...
<div class="main">
  ...
  <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:12px">
    {% if user.ip and geo %}
    <div class="card" style="padding:12px 16px;background:#020a14;border-color:#00d4ff22">
      <div style="font-size:10px;color:#2a4a6a;margin-bottom:6px">🌐 IP RÉELLE DU CLIENT</div>
      <span style="font-size:22px">{{ geo.flag }}</span>
      <span style="color:#00d4ff;font-weight:bold;font-size:16px;margin-left:10px">{{ user.ip }}</span>
      <div style="margin-top:6px">
        <span style="color:#4a7aaa;font-size:11px">📍 {{ geo.city }}, {{ geo.country }}</span>
        <span style="color:#2a4a6a;font-size:10px;margin-left:12px">ISP: {{ geo.isp }}</span>
      </div>
    </div>
    {% endif %}
    {% if user.os_info %}
    <div class="card" style="padding:12px 16px;background:#0a0020;border-color:#4422aa44">
      <div style="font-size:10px;color:#2a4a6a;margin-bottom:6px">💻 SYSTÈME D'EXPLOITATION CLIENT</div>
      <span style="color:#8866cc;font-size:13px;font-weight:bold">{{ user.os_info }}</span>
    </div>
    {% endif %}
  </div>
  ...
  <div class="detail-2col">
    <div class="card">
      ...
      <div class="detail-grid">
        ...
        <div class="detail-item"><div class="k">IP Réelle</div><div class="v">
          <span style="color:#00d4ff;font-weight:bold">{{ user.ip or '—' }}</span>
          {% if geo %}<span style="font-size:10px;color:#4a7aaa;display:block">{{ geo.flag }} {{ geo.city }}, {{ geo.country }}</span>{% endif %}
        </div></div>
        <div class="detail-item"><div class="k">OS Client</div><div class="v" style="color:#8866cc;font-size:10px">{{ user.os_info or '—' }}</div></div>
  ...
  <div class="card">
    ...
    <div class="actions-bar">
      ...
      {% if user.ip %}
      <a href="/admin/ips" class="btn danger">🚫 Bannir IP {{ user.ip }}</a>
      {% endif %}
    </div>
  </div>
...
<div class="main">
  <div class="page-header">
    ...
    <p>Les admins ne sont jamais bloqués. Seules les API login/register retournent une erreur aux clients.</p>
  </div>
...
<div class="main">
  ...
  <div class="card" style="padding:0">
    ...
    <table>
      <thead><tr><th>Username</th><th>Plan</th><th>Statut</th><th>IP Réelle</th><th>OS</th><th>Discord</th><th>Connexions</th><th>Dernière co.</th><th>Actions</th></tr></thead>
      <tbody>
        {% for u in results %}
        <tr>
          <td><a href="/admin/user/{{ u.username }}">{{ u.username }}</a></td>
          <td><span class="badge {{ u.plan }}">{{ u.plan }}</span></td>
          <td><span class="badge {{ u.status }}">{{ u.status }}</span></td>
          <td style="color:#00d4ff;font-size:10px;font-weight:bold">{{ u.ip or '—' }}</td>
          <td>{% if u.os_info %}<span class="os-badge">{{ u.os_info[:30] }}</span>{% else %}—{% endif %}</td>
          ...
          <td><a href="/admin/user/{{ u.username }}" class="btn primary" style="padding:3px 8px;font-size:9px">Voir</a></td>
        </tr>
        {% endfor %}
        {% if not results %}
        <tr><td colspan="9" style="text-align:center;color:#2a4a6a;padding:30px">Aucun résultat</td></tr>
        {% endif %}
      </tbody>
    </table>
  </div>
...
# ═══ PING — Keep-alive pour Render free tier (UptimeRobot) ═══════════════════
@app.route("/ping")
def ping():
    """Endpoint keep-alive — à pinguer toutes les 5 min via UptimeRobot."""
...
if __name__ == "__main__":
    ...
    print("\n" + "="*54)
    print("  ⚡ WINOPTIMIZER LICENSE SERVER v2.2")
    print("="*54)
    print(f"  Port             : {port}")
    print(f"  Login admin      : xywez / Admin2025!")
    print(f"  Base de données  : {DB_PATH}")
    print(f"  IP réelle        : CF-Connecting-IP > X-Real-IP > X-Forwarded-For")
    print(f"  OS client        : Envoyé par le logiciel au login")
    print("="*54)
