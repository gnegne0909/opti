...
def admin_resellers():
    conn = get_db()
    rows   = conn.execute("SELECT * FROM resellers ORDER BY joined_at DESC").fetchall()
    total  = len(rows)
    ...
    def _row(r):
        rp    = RESELLER_PLANS_SERVER.get(r["plan"], {})
        color = {"active":"b-green","pending":"b-yellow","suspended":"b-red","rejected":"b-gray"}.get(r["status"],"b-gray")
        return (f'<tr>'
                f'<td><b>{r["discord_tag"] or r["discord_id"]}</b><br><small style="color:var(--muted)">{r["discord_id"]}</small></td>'
                f'<td>{rp.get("name","?")}</td>'
                f'<td><span class="badge {color}">{r["status"]}</span></td>'
                f'<td style="font-weight:700">{r["credits"]:.2f}</td>'
                f'<td>{r["keys_generated"]}</td>'
                f'<td>{fmt_ts(r["joined_at"])}</td>'
                f'<td style="display:flex;gap:6px;flex-wrap:wrap">'
                f'<form method="POST" action="/admin/resellers/{r["discord_id"]}/credit"><input name="credits" placeholder="crédits" style="width:70px;background:var(--card2);border:1px solid var(--border);color:var(--text);padding:2px 6px;border-radius:4px"><button class="btn btn-green btn-xs">+</button></form>'
                f'<form method="POST" action="/admin/resellers/{r["discord_id"]}/toggle"><button class="btn btn-ghost btn-xs">{"Suspendre" if r["status"]=="active" else "Réactiver"}</button></form>'
                f'</td>'
                f'</tr>')

    rows_html = "".join(_row(r) for r in rows) or '<tr><td colspan="7" style="text-align:center;padding:20px;color:var(--muted)">Aucun revendeur</td></tr>'
    content = f"""
<div style="display:grid;grid-template-columns:repeat(3,1fr);gap:14px;margin-bottom:20px">
  <div class="card"><div style="color:var(--text2);font-size:11px">TOTAL</div><div style="font-size:28px;font-weight:800">{total}</div></div>
...
<div class="card">
  <div class="card-title">🤝 Revendeurs</div>
  <div class="tbl-wrap"><table class="table">
    <thead><tr><th>Discord</th><th>Plan</th><th>Statut</th><th>Crédits</th><th>Clés générées</th><th>Inscrit</th><th>Actions</th></tr></thead>
...
def reseller_required(f):
    ...
    def decorated(*args, **kwargs):
        ...
        if not row or row["status"] != "active":
            session.clear()
            return redirect("/reseller_panel/login?suspended=1")
        return f(*args, **kwargs)
    return decorated
...
def reseller_login():
    ...
    if request.method == "POST":
        ...
        if row and row["status"] == "active":
            expected = row.get("password") or (discord_id + "_wop")
