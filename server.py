...
SITE_CSS = """
<style>
@import url('https://fonts.googleapis.com/css2?family=Rajdhani:wght@400;500;600;700&family=Exo+2:ital,wght@0,300;0,400;0,500;0,600;0,700;0,800;0,900;1,400&display=swap');
*{box-sizing:border-box;margin:0;padding:0}
:root{
  --bg:#050507;--card:#0c0c10;--card2:#111116;--border:#1a1a24;
  --accent:#ff3d00;--accent2:#ff6d00;--accent-d:#1f0600;
  --green:#00e5a0;--red:#ef4444;--gold:#f59e0b;--blue:#3b82f6;
  --text:#f5f5fa;--text2:#7a7a96;--muted:#2e2e3a;--nav-h:62px;
}
html{scroll-behavior:smooth}
body{background:var(--bg);color:var(--text);font-family:'Exo 2',system-ui,sans-serif;overflow-x:hidden}

/* ────── NAV ────── */
.site-nav{position:fixed;top:0;left:0;right:0;height:var(--nav-h);z-index:100;
  background:rgba(5,5,7,.94);backdrop-filter:blur(18px);
  border-bottom:1px solid var(--border)}
.site-nav::after{content:'';position:absolute;bottom:0;left:0;width:100%;height:2px;
  background:linear-gradient(90deg,var(--accent),var(--accent2),transparent 70%)}
.nav-wrap{max-width:1300px;margin:auto;height:100%;display:flex;align-items:center;padding:0 22px;gap:24px}
.nav-logo{display:flex;align-items:center;gap:10px;text-decoration:none;flex-shrink:0}
.nav-logo-box{width:34px;height:34px;background:var(--accent-d);border:1px solid rgba(255,61,0,.4);
  border-radius:6px;display:flex;align-items:center;justify-content:center;font-size:14px;font-weight:900;color:var(--accent);
  font-family:'Rajdhani',sans-serif;position:relative;overflow:hidden}
.nav-logo-box::before{content:'';position:absolute;inset:0;background:linear-gradient(135deg,rgba(255,61,0,.2),transparent)}
.nav-logo-name{font-family:'Rajdhani',sans-serif;font-size:18px;font-weight:700;
...
.nav-a{text-decoration:none;color:var(--text2);font-size:13px;font-weight:600;
  padding:6px 14px;border-radius:5px;transition:.12s;text-transform:uppercase;letter-spacing:.06em;font-family:'Rajdhani',sans-serif}
.nav-a:hover,.nav-a.on{color:var(--text);background:rgba(255,61,0,.08)}
.nav-a.on{color:var(--accent2)}
.nav-actions{display:flex;gap:8px;align-items:center;flex-shrink:0}
.nav-btn{text-decoration:none;padding:6px 14px;border-radius:6px;font-size:12px;font-weight:700;
  transition:.12s;display:inline-flex;align-items:center;gap:5px;white-space:nowrap;
  text-transform:uppercase;letter-spacing:.06em;font-family:'Rajdhani',sans-serif;border:1px solid transparent}
.nav-btn:hover{transform:translateY(-1px)}
.nb-ghost{border-color:rgba(255,255,255,.1);color:var(--text2);background:transparent}
.nb-ghost:hover{border-color:rgba(255,61,0,.3);color:var(--accent2)}
.nb-fire{background:linear-gradient(135deg,var(--accent),var(--accent2));color:#fff;border:none}
.nb-fire:hover{box-shadow:0 4px 20px rgba(255,61,0,.4)}
.nb-admin{border-color:rgba(0,229,160,.2);color:var(--green);background:rgba(0,229,160,.05)}
.nb-res{border-color:rgba(124,58,237,.25);color:#a78bfa;background:rgba(124,58,237,.06)}
...
.mob-nav{display:none;flex-direction:column;gap:3px;padding:14px 16px 18px;position:fixed;
  top:var(--nav-h);left:0;right:0;background:rgba(5,5,7,.98);backdrop-filter:blur(18px);
  border-bottom:1px solid var(--border);z-index:99}
.mob-nav.open{display:flex}
.mob-a{text-decoration:none;color:var(--text2);padding:9px 12px;border-radius:6px;font-size:14px;
  font-weight:600;text-transform:uppercase;letter-spacing:.06em;font-family:'Rajdhani',sans-serif}
.mob-a:hover{background:rgba(255,61,0,.07);color:var(--text)}
...
/* Scanlines */
.h-scan{position:absolute;inset:0;
  background:repeating-linear-gradient(0deg,transparent,transparent 3px,rgba(255,61,0,.025) 3px,rgba(255,61,0,.025) 4px);
  animation:scan 8s linear infinite}
@keyframes scan{from{background-position:0 0}to{background-position:0 100px}}
/* Grid */
.h-grid{position:absolute;inset:0;
  background-image:linear-gradient(rgba(255,61,0,.05) 1px,transparent 1px),linear-gradient(90deg,rgba(255,61,0,.05) 1px,transparent 1px);
  background-size:50px 50px;mask-image:radial-gradient(ellipse 80% 80% at 50% 50%,black,transparent)}
/* Glow */
.h-glow{position:absolute;width:800px;height:800px;border-radius:50%;
  background:radial-gradient(circle,rgba(255,61,0,.12) 0%,transparent 60%);
  top:50%;left:50%;transform:translate(-50%,-50%);animation:gp 4s ease-in-out infinite}
@keyframes gp{0%,100%{opacity:.7;transform:translate(-50%,-50%) scale(1)}50%{opacity:1;transform:translate(-50%,-50%) scale(1.08)}}
/* Diagonal accent line */
.h-line{position:absolute;top:0;right:0;width:50%;height:100%;
  background:linear-gradient(135deg,transparent 40%,rgba(255,61,0,.04) 100%);pointer-events:none}
.hero-inner{position:relative;text-align:center;max-width:900px;z-index:1}
.h-eyebrow{display:inline-flex;align-items:center;gap:8px;
  background:rgba(255,61,0,.08);color:var(--accent2);border:1px solid rgba(255,61,0,.2);
  border-radius:3px;padding:5px 14px;font-size:11px;font-weight:700;
  text-transform:uppercase;letter-spacing:.12em;margin-bottom:22px;font-family:'Rajdhani',sans-serif}
.h-eyebrow i{width:6px;height:6px;background:var(--accent);border-radius:50%;display:inline-block;animation:bk 1.5s infinite}
@keyframes bk{0%,100%{opacity:1}50%{opacity:.2}}
.h-title{font-family:'Rajdhani',sans-serif;font-size:clamp(56px,9vw,108px);font-weight:700;
  line-height:.92;letter-spacing:-.01em;text-transform:uppercase;margin-bottom:20px}
.h-title .fire{color:var(--accent);position:relative;display:inline-block}
.h-title .fire::after{content:attr(data-text);position:absolute;left:0;top:0;color:var(--accent2);
  clip-path:polygon(0 0,100% 0,100% 40%,0 40%);opacity:.5}
.h-sub{font-size:clamp(15px,2vw,18px);color:var(--text2);max-width:520px;margin:0 auto 36px;line-height:1.65;font-weight:400}
.h-btns{display:flex;gap:14px;justify-content:center;flex-wrap:wrap;margin-bottom:52px}

/* Hero stats — horizontal bar style */
.h-stats{display:grid;grid-template-columns:repeat(4,1fr);border:1px solid var(--border);border-radius:8px;overflow:hidden;background:var(--card)}
.h-stat{padding:18px 20px;border-right:1px solid var(--border);text-align:center;position:relative}
.h-stat:last-child{border-right:none}
.h-stat::before{content:'';position:absolute;top:0;left:0;right:0;height:2px;background:linear-gradient(90deg,var(--accent),transparent)}
.h-stat-n{font-family:'Rajdhani',sans-serif;font-size:38px;font-weight:700;color:var(--accent2);line-height:1}
.h-stat-l{font-size:11px;color:var(--text2);margin-top:4px;text-transform:uppercase;letter-spacing:.08em;font-weight:600;font-family:'Rajdhani',sans-serif}

/* ────── SECTION ────── */
.section{max-width:1300px;margin:auto;padding:80px 22px}
.sec-tag{display:inline-flex;align-items:center;gap:6px;
  background:rgba(255,61,0,.07);color:var(--accent2);border-left:2px solid var(--accent);
  padding:4px 12px;font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:.12em;margin-bottom:14px;font-family:'Rajdhani',sans-serif}
.sec-title{font-family:'Rajdhani',sans-serif;font-size:clamp(30px,4vw,58px);font-weight:700;
  line-height:.95;letter-spacing:.01em;text-transform:uppercase}
.sec-sub{font-size:15px;color:var(--text2);margin-top:12px;line-height:1.65;max-width:520px}
.fire-text{color:var(--accent)}

/* ────── BUTTONS (public) ────── */
.btn{display:inline-flex;align-items:center;gap:8px;padding:13px 28px;border-radius:7px;
  font-size:14px;font-weight:700;text-decoration:none;transition:.15s;cursor:pointer;border:none;
  font-family:'Rajdhani',sans-serif;text-transform:uppercase;letter-spacing:.06em}
.btn:hover{transform:translateY(-2px);text-decoration:none}
.btn-fire{background:linear-gradient(135deg,var(--accent),var(--accent2));color:#fff}
.btn-fire:hover{box-shadow:0 8px 30px rgba(255,61,0,.4)}
.btn-outline{background:transparent;color:var(--text);border:1px solid var(--border)}
.btn-outline:hover{border-color:var(--accent2);color:var(--accent2)}
.btn-ghost{background:rgba(255,255,255,.04);color:var(--text2);border:1px solid var(--border)}
.btn-ghost:hover{background:rgba(255,61,0,.06);color:var(--text);border-color:rgba(255,61,0,.2)}
.btn-sm{padding:8px 18px;font-size:13px;border-radius:6px}

/* ────── FEATURE CARDS ────── */
.feat-card{background:var(--card);border:1px solid var(--border);border-radius:8px;padding:24px;
  transition:.15s;position:relative;overflow:hidden}
.feat-card::before{content:'';position:absolute;top:0;left:0;width:2px;height:100%;
  background:linear-gradient(180deg,var(--accent),transparent)}
.feat-card:hover{border-color:rgba(255,61,0,.2);transform:translateY(-3px);box-shadow:0 12px 40px rgba(0,0,0,.4)}
.feat-icon{width:42px;height:42px;border-radius:8px;display:flex;align-items:center;
  justify-content:center;font-size:20px;margin-bottom:14px;position:relative}
.feat-title{font-size:15px;font-weight:700;margin-bottom:8px;font-family:'Rajdhani',sans-serif;
  text-transform:uppercase;letter-spacing:.04em}
.feat-desc{font-size:13px;color:var(--text2);line-height:1.6}

/* ────── PLAN CARDS ────── */
.plan-card{background:var(--card);border:1px solid var(--border);border-radius:8px;padding:28px;
  position:relative;transition:.15s;overflow:hidden}
.plan-card.featured{border-color:rgba(255,61,0,.35)}
.plan-card.featured::before{content:'';position:absolute;top:0;left:0;right:0;height:2px;
  background:linear-gradient(90deg,var(--accent),var(--accent2))}
.plan-badge{position:absolute;top:-1px;right:20px;
  background:linear-gradient(135deg,var(--accent),var(--accent2));color:#fff;
  padding:3px 12px;border-radius:0 0 6px 6px;font-size:10px;font-weight:700;
  text-transform:uppercase;letter-spacing:.08em;font-family:'Rajdhani',sans-serif}
.plan-name{font-family:'Rajdhani',sans-serif;font-size:12px;font-weight:700;
  text-transform:uppercase;letter-spacing:.12em;color:var(--text2);margin-bottom:8px}
.plan-price{font-family:'Rajdhani',sans-serif;font-size:62px;font-weight:700;line-height:1;letter-spacing:-.02em}
.plan-price sup{font-size:26px;font-weight:700;vertical-align:top;margin-top:10px}
.plan-period{font-size:13px;color:var(--text2);margin-top:4px;font-family:'Rajdhani',sans-serif}
.plan-features{list-style:none;margin:20px 0 22px;display:flex;flex-direction:column;gap:9px}
.plan-features li{display:flex;align-items:flex-start;gap:9px;font-size:13px}
.plan-features li.yes{color:var(--text)}
.plan-features li.yes::before{content:"✓";color:var(--green);font-weight:700;flex-shrink:0;margin-top:1px;font-family:'Rajdhani',sans-serif}
.plan-features li.no{color:var(--muted)}
.plan-features li.no::before{content:"—";flex-shrink:0}

/* ────── REVIEWS ────── */
.review-card{background:var(--card);border:1px solid var(--border);border-radius:8px;padding:22px;transition:.15s}
.review-card:hover{border-color:rgba(255,61,0,.15)}
.review-stars{color:var(--gold);font-size:14px;margin-bottom:10px;letter-spacing:2px}
.review-text{font-size:13px;color:var(--text2);line-height:1.7;margin-bottom:14px;font-style:italic}
.review-author{font-size:13px;font-weight:700;display:flex;align-items:center;gap:9px}
.review-av{width:32px;height:32px;border-radius:6px;
...
.footer-wrap{max-width:1300px;margin:auto;padding:48px 22px;
  display:grid;grid-template-columns:2fr 1fr 1fr 1fr;gap:40px}
.footer-brand-name{font-family:'Rajdhani',sans-serif;font-size:22px;font-weight:700;
  text-transform:uppercase;letter-spacing:.1em;margin-bottom:8px}
.footer-brand-name b{color:var(--accent)}
.footer-brand-sub{font-size:13px;color:var(--text2);line-height:1.6;max-width:240px;margin-bottom:18px}
...
.footer-sb{background:var(--card2);border:1px solid var(--border);color:var(--text2);
  ...
  transition:.12s;font-family:'Rajdhani',sans-serif;text-transform:uppercase;letter-spacing:.06em}
.footer-sb:hover{border-color:var(--accent);color:var(--accent2)}
.footer-col-title{font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:.12em;
  color:var(--muted);margin-bottom:12px;font-family:'Rajdhani',sans-serif}
.footer-col{display:flex;flex-direction:column;gap:9px}
.footer-col a{text-decoration:none;color:var(--text2);font-size:13px;transition:.1s}
.footer-col a:hover{color:var(--text)}
.footer-bottom{max-width:1300px;margin:auto;padding:14px 22px;border-top:1px solid var(--border);
  display:flex;justify-content:space-between;align-items:center;font-size:12px;color:var(--muted)}

/* ────── BADGES ────── */
.badge{display:inline-flex;align-items:center;padding:2px 8px;border-radius:3px;font-size:11px;font-weight:700;font-family:'Rajdhani',sans-serif;letter-spacing:.04em}
.b-green{background:rgba(0,229,160,.08);color:var(--green);border:1px solid rgba(0,229,160,.2)}
.b-red{background:rgba(239,68,68,.08);color:var(--red);border:1px solid rgba(239,68,68,.2)}
.b-yellow{background:rgba(245,158,11,.08);color:var(--gold);border:1px solid rgba(245,158,11,.2)}
.b-purple{background:rgba(124,58,237,.08);color:#a78bfa;border:1px solid rgba(124,58,237,.2)}
.b-gray{background:rgba(255,255,255,.04);color:var(--text2);border:1px solid var(--border)}
.form-group{margin-bottom:14px}
.form-group label{display:block;font-size:11px;font-weight:700;color:var(--text2);margin-bottom:5px;
  text-transform:uppercase;letter-spacing:.06em;font-family:'Rajdhani',sans-serif}
.form-group input,.form-group select,.form-group textarea{width:100%;background:var(--card2);border:1px solid var(--border);
  color:var(--text);padding:10px 14px;border-radius:6px;font-size:14px;outline:none;transition:.12s;font-family:'Exo 2',sans-serif}
...
@media(max-width:768px){
  ...
  .nav-burger{display:flex}.section{padding:52px 16px}
  .h-stats{grid-template-columns:1fr 1fr}.h-stat{border-right:none;border-bottom:1px solid var(--border)}
  .h-stat:nth-child(2n){border-right:none}.h-stat:nth-child(3),.h-stat:nth-child(4){border-bottom:none}
}
@media(max-width:480px){.section{padding:36px 14px}.h-btns{flex-direction:column;align-items:stretch}}

/* ────── ANIMATIONS ────── */
@keyframes fade-up{from{opacity:0;transform:translateY(20px)}to{opacity:1;transform:translateY(0)}}
.anim{animation:fade-up .5s ease both}
.d1{animation-delay:.1s}.d2{animation-delay:.2s}.d3{animation-delay:.3s}.d4{animation-delay:.45s}

/* ────── TABLE (public) ────── */
.table{width:100%;border-collapse:collapse;font-size:13px}
.table th{text-align:left;padding:9px 14px;color:var(--text2);font-size:10px;font-weight:700;
  text-transform:uppercase;letter-spacing:.08em;border-bottom:1px solid var(--border);font-family:'Rajdhani',sans-serif}
.table td{padding:11px 14px;border-bottom:1px solid rgba(255,255,255,.04)}
.table tr:hover td{background:rgba(255,61,0,.03)}
.tbl-wrap{overflow-x:auto}

/* ────── FAQ ────── */
.faq-item{border:1px solid var(--border);border-radius:8px;overflow:hidden;margin-bottom:8px;transition:.15s}
...
.faq-q{padding:16px 20px;font-size:14px;font-weight:600;cursor:pointer;display:flex;
  justify-content:space-between;align-items:center;transition:.12s;font-family:'Rajdhani',sans-serif;
  text-transform:uppercase;letter-spacing:.04em}
.faq-q:hover{background:rgba(255,61,0,.04)}
...
/* ────── CHATBOT colors ────── */
.quick-btn{background:rgba(255,61,0,.08);border:1px solid rgba(255,61,0,.2);color:var(--accent2);
  border-radius:4px;padding:5px 11px;font-size:12px;cursor:pointer;transition:.1s;white-space:nowrap;font-family:'Rajdhani',sans-serif}
.quick-btn:hover{background:rgba(255,61,0,.16)}
</style>
"""
...
def site_home():
    ...
    ico_b64 = ""  # TODO: replace with base64-encoded favicon if desired
    page = f"""<!DOCTYPE html>
<html lang="fr"><head>
<meta charset="utf-8">
...
<link rel="icon" type="image/x-icon" href="data:image/x-icon;base64,{ico_b64}">
{SITE_CSS}
</head><body>
{_site_nav("home")}
<div class="page">
...
<section class="hero">
  ...
  <div class="hero-inner">
    <div class="h-eyebrow anim"><i></i> +5 000 gamers optimisés · Mis à jour chaque semaine</div>
    <h1 class="h-title anim d1">
      DOMINATION<br><span class="fire" data-text="TOTALE">TOTALE</span><br>DES FPS
    </h1>
    <p class="h-sub anim d2">
      WinOptimizer Pro extrait chaque frame et chaque milliseconde de ton setup.
      Installation en 2 minutes. Résultats immédiats.
    </p>
    <div class="h-btns anim d3">
      <a href="/pricing" class="btn btn-fire" style="padding:15px 38px;font-size:16px">Commencer →</a>
      <a href="/features" class="btn btn-outline" style="padding:15px 28px;font-size:16px">Fonctionnalités</a>
    </div>
    <div class="h-stats anim d4">
...
<!-- ░░ FEATURES ░░ -->
<section style="background:linear-gradient(180deg,var(--bg),rgba(255,61,0,.03),var(--bg))">
  <div class="section">
    <div style="text-align:center;margin-bottom:52px">
      <div class="sec-tag">Fonctionnalités</div>
      <h2 class="sec-title" style="text-align:center">TOUT CE QU'IL FAUT<br>POUR <span class="fire-text">DOMINER</span></h2>
      <p class="sec-sub" style="text-align:center;margin:12px auto 0">Un seul outil, toutes les optimisations. Aucune compétence technique requise.</p>
    </div>
    <div class="grid-4">{feats_html}</div>
    <div style="text-align:center;margin-top:36px">
      <a href="/features" class="btn btn-outline">Voir toutes les fonctionnalités →</a>
    </div>
  </div>
</section>
...
<section>
  <div class="section">
    <div style="text-align:center;margin-bottom:52px">
      ...
      <h2 class="sec-title" style="text-align:center">SIMPLE &amp; <span class="fire-text">TRANSPARENT</span></h2>
      <p class="sec-sub" style="text-align:center;margin:12px auto 0">Paiement unique. Aucun abonnement.</p>
    </div>
    <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:20px;max-width:1000px;margin:auto;align-items:start">
      ...
      <div class="plan-card anim d1">
        ...
        <ul class="plan-features">
          ...
          <li class="no">Support à distance</li>
        </ul>
      ...
      <div class="plan-card featured anim d2">
        ...
        <div class="plan-price" style="color:var(--accent2)"><sup>€</sup>{price_p:.0f}</div>
        <div class="plan-period">Paiement unique · À vie</div>
        <ul class="plan-features">
          ...
          <li class="yes">Consultation IA intégrée</li>
          <li class="yes">Support à distance</li>
          <li class="yes">Accès prioritaire bêta</li>
          <li class="yes">Badge PRO Discord</li>
        </ul>
      ...
      <div class="plan-card anim d3">
        ...
        <div class="plan-price" style="color:#a78bfa;font-size:42px">60–100€</div>
        ...
        <ul class="plan-features">
          ...
          <li class="yes">Génération clés NORMAL/PRO</li>
          <li class="yes">Stats revenus en temps réel</li>
          <li class="yes">Support revendeur dédié</li>
        </ul>
        <a href="{discord_link}" target="_blank" class="btn btn-outline" style="width:100%;justify-content:center;border-color:rgba(124,58,237,.3);color:#a78bfa">Devenir Revendeur →</a>
      </div>
    </div>
  </div>
</section>
...
<section>
  <div class="section">
    <div style="text-align:center;margin-bottom:48px">
      <div class="sec-tag">Avis clients</div>
      <h2 class="sec-title" style="text-align:center">CE QU'ILS EN <span class="fire-text">PENSENT</span></h2>
    </div>
    <div class="grid-3">{reviews_html}</div>
    <div style="text-align:center;margin-top:28px">
      <a href="/client#review" class="btn btn-ghost btn-sm">✍ Laisser un avis (espace client)</a>
    </div>
  </div>
</section>
...
<section>
  <div class="section" style="padding-top:0">
    <div style="background:var(--card);border:1px solid rgba(255,61,0,.25);border-radius:8px;
      padding:52px 32px;text-align:center;position:relative;overflow:hidden">
      <div style="position:absolute;top:0;left:0;right:0;height:2px;background:linear-gradient(90deg,var(--accent),var(--accent2),transparent)"></div>
      <h2 class="sec-title" style="text-align:center">PRÊT À <span class="fire-text">TOUT DÉCHIRER</span> ?</h2>
      <p style="font-size:16px;color:var(--text2);margin:16px auto 32px;max-width:440px;line-height:1.65">
        Rejoins +5 000 gamers qui ont transformé leur PC en machine de guerre.
      </p>
      <div style="display:flex;gap:14px;justify-content:center;flex-wrap:wrap">
        <a href="/pricing" class="btn btn-fire" style="padding:14px 38px;font-size:16px">Acheter maintenant →</a>
        <a href="{discord_link}" target="_blank" class="btn btn-outline" style="padding:14px 28px;font-size:16px">💬 Discord</a>
      </div>
    </div>
  </div>
</section>
