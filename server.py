...
def ping():
    maintenance = get_setting("maintenance") == "1"
    return jsonify({"status": "maintenance" if maintenance else "ok", "message": get_setting("maintenance_msg") if maintenance else "Serveur opérationnel", "version": APP_VERSION, "ts": int(time.time())}), 200
